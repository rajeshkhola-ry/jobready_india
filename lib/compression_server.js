...
function convertSpreadsheetWithLibreOffice(inputPath, targetFormat, callback) {
  ...
  acquireLibreOfficeSlot(function () {
    var convertFilter = targetFormat === 'pdf' ? 'pdf:calc_pdf_Export' : 'csv:Text - txt - csv (StarCalc)';
    var args = [
      '--headless',
      '--norestore',
      '--invisible',
...
// archive (rasterized via Ghostscript), or Excel (XLSX/XLS) -> CSV/PDF via
// headless LibreOffice - all server-side so large/complex files never need
// to be parsed or rendered in browser memory.
app.post('/api/convert', upload.single('file'), enforceQuotaMiddleware, function (req, res) {
  ...
  try {
    ...
    var isSpreadsheetInput = resolvedFileType === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' ||
      resolvedFileType === 'application/vnd.ms-excel';

    if (isSpreadsheetInput) {
      var sheetTargetFormat = (req.body.targetFormat || 'csv').toLowerCase();
      if (['csv', 'pdf'].indexOf(sheetTargetFormat) === -1) {
        cleanupFile(req.file.path);
        return res.status(400).json({ success: false, error: 'targetFormat must be csv or pdf for Excel input' });
      }
      ...
      convertSpreadsheetWithLibreOffice(tempInputPath, sheetTargetFormat, function (err, outputBytes) {
        ...
        logAuditEvent(req.body && req.body.planId ? req.body.planId : 'guest', 'tool-convert-xlsx-' + sheetTargetFormat, { tool: 'convert', planId: req.quotaContext && req.quotaContext.planId ? req.quotaContext.planId : '' });

        res.set('Content-Type', sheetTargetFormat === 'pdf' ? 'application/pdf' : 'text/csv');
        res.set('Content-Disposition', 'attachment; filename="' + sheetBaseName + '.' + sheetTargetFormat + '"');
    ...
    if (resolvedFileType !== 'application/pdf') {
      cleanupFile(req.file.path);
      return res.status(400).json({ success: false, error: 'Only PDF or Excel (XLSX/XLS) files are supported for this endpoint.' });
    }
