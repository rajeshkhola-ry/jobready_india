...
async function resendAdminEmailOtpChallenge(challenge) {
  ...
  var otpCode = createEmailOtpCode();
  challenge.otpHash = hashEmailOtp(token, otpCode);
  challenge.createdAt = Date.now();
  challenge.expiresAt = challenge.createdAt + Math.max(60, ADMIN_CONFIG.twoFactorChallengeTtlSec) * 1000;
...
app.post('/api/admin/2fa/resend', async function (req, res) {
  ...
  if (!resendPayload || resendPayload.success === false) {
    return res.status(500).json({ success: false, error: 'Unable to resend OTP right now. Please try again.' });
  }
