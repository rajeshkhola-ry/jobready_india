...
function getInvoicePosContext(billing) {
  ...
  var stateCode = resolveStateCode(stateName) || (countryName === 'India' ? sellerStateCode : '');
  var isIntrastate = countryName === 'India' && stateCode === sellerStateCode;
  return {
    country: countryName,
    ...
    gstType: isIntrastate ? 'CGST + SGST' : (countryName === 'India' ? 'IGST' : 'GST')
  };
}
...
// Single source of truth for B2B/B2C/SEZ tagging and the CGST+SGST vs IGST split.
function resolveGstClassification(billing) {
  ...
  return {
    ...
    customerGstin: hasValidGstin ? gstin : '',
    useIgst: useIgst,
    gstType: !isDomestic ? 'GST' : (useIgst ? 'IGST' : 'CGST + SGST')
  };
}
...
function resolveTaxBreakdown(totalPaidAmount, billing, currency) {
  ...
  return {
    ...
    isSez: classification.isSez,
    transactionType: classification.transactionType,
    gstType: isDomestic ? (useIgst ? 'IGST' : 'CGST + SGST') : 'GST',
