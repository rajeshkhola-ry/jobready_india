...
// ---------------------------------------------------------------------------
// PDF -> DOCX final fallback (Tier 3): for scanned/image-only PDFs where both
// pdf2docx and LibreOffice's native PDF import fail (malformed scanner-app
// metadata, unusual embedded image encodings, etc). Rasterizes every page to
// a JPEG via Ghostscript, then packages the images directly into a DOCX via
// python-docx (rasterize_to_docx.py) - NOT via LibreOffice's HTML import,
// which can fail to embed file:// or relative image URIs when run headless.
// This guarantees the user always gets a valid, openable Word document (page
// images, not searchable text, since there is no text to recover from a pure
// scan) instead of a hard failure. Every path cleans up its own temp dirs.
// ---------------------------------------------------------------------------
function convertPdfToDocxFromRasterizedImages(inputPath, callback) {
  var execFile = require('child_process').execFile;
  var gsCommand = resolveGhostscriptBinary();
  var jobId = 'p2drasf_' + Date.now() + '_' + Math.round(Math.random() * 1e6);
  var pageDir = path.join(UPLOAD_DIR, 'p2drasf_pages_' + jobId);
  var outDir = path.join(UPLOAD_DIR, 'p2drasf_out_' + jobId);
  ...
  try {
    ...
    console.error('[convert-pdf-to-docx][Tier3:RasterImages] Cannot create work directories: ' + dirErr.message);
    return callback(new Error('Cannot create fallback work directories: ' + dirErr.message));
  }
  ...
  var gsArgs = [
    ...
    '-r150',
    '-dJPEGQ=85',
    // Caps Ghostscript's internal bitmap cache so large/high-page-count PDFs
    // render in bands instead of holding a full uncompressed page bitmap in
    // memory at once - keeps peak RAM well under control for 20+ page scans.
    '-dMaxBitmap=8388608',
    '-o', path.join(pageDir, 'page_%d.jpg'),
    inputPath
  ];

  execFile(gsCommand, gsArgs, { timeout: 80000, maxBuffer: 20 * 1024 * 1024 }, function (gsErr, gsStdout, gsStderr) {
    if (gsErr) {
      console.error('[convert-pdf-to-docx][Tier3:RasterImages] Ghostscript rasterization failed: ' + gsErr.message + (gsStderr ? (' | stderr: ' + gsStderr) : ''));
      cleanupAll();
      return callback(new Error('Could not rasterize this PDF for fallback conversion: ' + gsErr.message));
    }
    ...
    try {
      ...
      console.error('[convert-pdf-to-docx][Tier3:RasterImages] Cannot read rasterized pages: ' + readDirErr.message);
      cleanupAll();
      return callback(new Error('Cannot read rasterized pages: ' + readDirErr.message));
    }

    if (pageFiles.length === 0) {
      console.error('[convert-pdf-to-docx][Tier3:RasterImages] No pages were rasterized from this PDF.');
      cleanupAll();
      return callback(new Error('No pages could be rasterized from this PDF.'));
    }

    var outputPath = path.join(outDir, 'document.docx');
    var pythonBin = resolvePython3Binary();
    var scriptPath = path.join(__dirname, 'rasterize_to_docx.py');
    var scriptArgs = [scriptPath, outputPath].concat(pageFiles.map(function (fileName) {
      return path.join(pageDir, fileName);
    }));

    execFile(pythonBin, scriptArgs, { timeout: 60000, maxBuffer: 20 * 1024 * 1024 }, function (docxErr, docxStdout, docxStderr) {
      if (docxErr) {
        console.error('[convert-pdf-to-docx][Tier3:RasterImages] python-docx packaging failed: ' + docxErr.message + (docxStderr ? (' | stderr: ' + docxStderr) : '') + (docxStdout ? (' | stdout: ' + docxStdout) : ''));
        cleanupAll();
        return callback(new Error('Fallback image-based DOCX conversion failed: ' + docxErr.message));
      }

      var parsed = null;
      try { parsed = JSON.parse(docxStdout); } catch (parseErr) { parsed = null; }
      if (parsed && parsed.error) {
        console.error('[convert-pdf-to-docx][Tier3:RasterImages] python-docx reported error: ' + parsed.error);
        cleanupAll();
        return callback(new Error('Fallback image-based DOCX conversion failed: ' + parsed.error));
      }

      if (!fs.existsSync(outputPath) || fileSize(outputPath) === 0) {
        console.error('[convert-pdf-to-docx][Tier3:RasterImages] No DOCX output was produced. stdout: ' + docxStdout);
        cleanupAll();
        return callback(new Error('Fallback conversion did not produce a DOCX output.'));
      }
      ...
      try {
        ...
        console.error('[convert-pdf-to-docx][Tier3:RasterImages] Cannot read fallback DOCX output: ' + readErr.message);
        cleanupAll();
        return callback(new Error('Cannot read fallback DOCX output: ' + readErr.message));
      }
...
// Used to fast-path pure scans directly to Tier 3, skipping the ~150s worth
// of pdf2docx + LibreOffice attempts that are guaranteed to fail on a page
// with zero selectable text anyway. Always "fails open" (hasText: true) on
// any error so a broken check never blocks the normal tier chain.
function checkPdfHasSelectableText(inputPath, callback) {
  ...
  execFile(pythonBin, [scriptPath, inputPath], { timeout: 15000, maxBuffer: 5 * 1024 * 1024 }, function (err, stdout, stderr) {
    if (err) {
      console.error('[convert-pdf-to-docx][FastPath:TextCheck] Check failed, proceeding with full tier chain: ' + err.message + (stderr ? (' | stderr: ' + stderr) : ''));
    ...
    if (!parsed || parsed.error) {
      console.error('[convert-pdf-to-docx][FastPath:TextCheck] Check reported an error, proceeding with full tier chain: ' + (parsed && parsed.error ? parsed.error : 'unparseable output'));
      return callback({ hasText: true, checkFailed: true });
    }
...
// LibreOffice, then to a rasterized-image DOCX, so the endpoint always has a
// working path even for scanned/image-only PDFs with no selectable text.
// Pure scans (no text layer) skip straight to Tier 3 - see
// checkPdfHasSelectableText() above - since Tier 1/2 are guaranteed to fail
// on them and would otherwise waste ~150s of wall clock plus a full
// LibreOffice process spawn, which is what caused Render's proxy to drop
// the connection on large multi-page scanned documents.
function convertPdfToDocxHighFidelity(inputPath, callback) {
  checkPdfHasSelectableText(inputPath, function (textInfo) {
    var likelyScanned = textInfo && textInfo.hasText === false && !textInfo.checkFailed;

    if (likelyScanned) {
      console.error('[convert-pdf-to-docx][FastPath:ScannedPDF] Detected image-only/scanned PDF (textLength=' + textInfo.textLength + ' across ' + textInfo.sampledPages + '/' + textInfo.pageCount + ' sampled pages) - skipping Tier 1/2, jumping directly to Tier 3.');
      return convertPdfToDocxFromRasterizedImages(inputPath, function (rasterErr, rasterDocxBytes) {
        if (rasterErr) {
          console.error('[convert-pdf-to-docx][Tier3:RasterImages] Failed: ' + rasterErr.message + ' - all conversion tiers exhausted (fast-pathed, Tier 1/2 skipped).');
          return callback(rasterErr);
        }
        callback(null, rasterDocxBytes, 'scanned-image-fastpath');
      });
    }
    ...
    convertPdfToDocxWithPdf2docx(inputPath, function (pdf2docxErr, docxBytes) {
      ...
      convertPdfToDocxWithLibreOffice(inputPath, function (loErr, loDocxBytes) {
        ...
        console.error('[convert-pdf-to-docx][Tier2:LibreOffice] Failed: ' + (loErr ? loErr.message : 'unknown error') + ' - falling back to Tier 3 (RasterImages).');

        convertPdfToDocxFromRasterizedImages(inputPath, function (rasterErr, rasterDocxBytes) {
          if (rasterErr) {
            console.error('[convert-pdf-to-docx][Tier3:RasterImages] Failed: ' + rasterErr.message + ' - all conversion tiers exhausted.');
            return callback(rasterErr);
          }
          callback(null, rasterDocxBytes, 'scanned-image-fallback');
        });
      });
    });
  });
  ...
  try {
    ...
    convertPdfToDocxHighFidelity(tempInputPath, function (err, docxBytes, engine) {
      ...
      if (err) {
        ...
        console.error('[convert-pdf-to-docx] All conversion tiers failed for "' + (req.file && req.file.originalname) + '": ' + msg);
