...
// text. Multi-row grids (salary breakdowns, offer-letter tables) are
// detected first and rendered as real <w:tbl> structures; every other line
// keeps the existing single-line paragraph/right-tab-anchor behaviour.
function buildPageParagraphsXml(ocrPage, pageWidthPx) {
  ...
  var lines = clusterWordsIntoLines(words);
  var avgWordHeight = words.reduce(function (sum, w) { return sum + Math.max(1, w.y1 - w.y0); }, 0) / words.length;
  var tableRuns = detectTableRuns(lines, pageWidthPx);
  ...
  while (lineIndex < lines.length) {
    ...
    if (run && run.startLineIndex === lineIndex) {
      output += buildTableXml(run, pageWidthPx);
      lineIndex = run.endLineIndex + 1;
      runIndex += 1;
      continue;
    }
    output += buildLineParagraphXml(lines[lineIndex], pageWidthPx, avgWordHeight);
    lineIndex += 1;
  }
