...
function getCreditDebitNoteReportRow(note) {
  ...
  var gstin = String(billing.gstin || '').trim();
  var customerType = gstin ? 'B2B' : 'B2C';
  var stateName = String(billing.state || 'Delhi').trim() || 'Delhi';
  var posCode = resolveStateCode(stateName) || '07';
  var placeOfSupply = stateName + ' (' + posCode + ')';
  var sezStatus = String(billing.sez || billing.sezStatus || 'NO').trim().toUpperCase();
  ...
  try {
    ...
    if (!transaction) {
      ...
      if (receipt && !transaction.invoiceNumber) {
        transaction.invoiceNumber = receipt;
      }
