  ...
  checkPdfHasSelectableText(inputPath, function (textInfo) {
    ...
    convertPdfToDocxWithPdf2docx(inputPath, function (pdf2docxErr, docxBytes) {
      ...
      convertPdfToDocxWithLibreOffice(inputPath, function (loErr, loDocxBytes) {
        ...
        // Tier 3: reuse the shared Vision OCR engine as a final, LibreOffice-
        // independent fallback whenever BOTH pdf2docx and LibreOffice fail for
        // ANY reason (missing binary, parse error, timeout, etc.) - this is
        // what actually satisfies "never dead-end on missing LibreOffice" for
        // documents that were NOT confidently pre-detected as pure scans
        // above. Only attempted if the API key is configured and the shared
        // global monthly OCR page cap has room; otherwise fails through to
        // the original (now more informative) Tier 2 error unchanged.
        if (!resolveGoogleVisionApiKey()) {
          ...
          return callback(loErr || new Error('PDF to Word conversion failed.'));
        }
        if (!pageCount) {
        ...
        ocrScannedPdfToDocx(inputPath, visionOutputPath, function (ocrErr) {
          ...
          try {
            ...
            return callback(loErr || readErr);
          }

          removeWorkDir(outDir);
          callback(null, ocrDocxBytes, 'vision-ocr');
        });
      });
    });
  });
