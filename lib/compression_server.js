...
// Runs DOCUMENT_TEXT_DETECTION across all rasterized pages in ONE batched
// Vision API call (the `requests` array lets Vision process every page in a
// single HTTP round trip), returning per-page {text, words:[{text,x0,y0,x1,
// y1}]} results (pixel coordinates) in page order.
function runVisionOcrOnPages(pages, callback) {
  var requests;
  try {
    requests = pages.map(function (page) {
  ...
  callGoogleVisionAnnotate(requests, function (err, result) {
    ...
    var pageResults = responses.map(function (response) {
      if (response.error) {
        return { text: '', words: [] };
      }
