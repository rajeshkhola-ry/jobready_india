...
var planNameMap = {
  '7-day': '7 Days Access',
  'short-access': '7 Days Access',
  'monthly': '1 Month Pro',
  'yearly': '1 Year Unlimited Access',
  'lifetime-pro': 'Lifetime Pro'
};

function resolvePlanTitle(planId, rawPlanName) {
  var key = String(planId || '').trim().toLowerCase();
  return planNameMap[key] || rawPlanName || 'Get Ready Job Subscription';
}
...
function buildInvoicePdfBuffer(transaction) {
  ...
  var finalPlanTitle = resolvePlanTitle(transaction.planId, transaction.planName);
  var isDomesticInvoice = taxBreakdown.isDomestic !== undefined
    ? Boolean(taxBreakdown.isDomestic)
    : String(transaction.currency || 'INR').toUpperCase() === 'INR';
  var totalPaid = Number(transaction.totalAmount || transaction.amount || 0);
  var baseAmount = isDomesticInvoice ? Number((totalPaid / 1.18).toFixed(2)) : totalPaid;
  var taxAmount = isDomesticInvoice ? Number((totalPaid - baseAmount).toFixed(2)) : 0;
  ...
  var receiptLines = [
    // Header & branding
    { text: 'GET READY JOB', size: 20, center: true, gap: 26 },
    { text: 'Get Ready Job - Tax Invoice / Receipt', size: 13, center: true, gap: 18 },
    { text: 'Thanks for your purchase! Questions? Contact hello@getreadyjob.com', size: 9, center: true, gap: 32 },
    // Metadata
    { text: 'Date: ' + formatIstReceiptTimestamp(transaction.paidAt || transaction.createdAt), size: 10, gap: 16 },
    { text: 'Account Billed: ' + (billing.name ? billing.name : 'Guest User') + ' (' + (billing.email ? billing.email : 'no-email-provided') + ')', size: 10, gap: 16 },
    { text: 'Transaction ID: ' + (transaction.paymentId || transaction.transactionId), size: 10, gap: 16 },
    { text: 'Seller Profile: ' + seller.companyName + ' (Proprietorship: ' + seller.proprietorName + ')', size: 10, gap: 16 },
    { text: 'Seller Credentials: PAN: ' + seller.pan + ' | SAC Code: ' + seller.sacCode, size: 10, gap: 16 },
    { text: 'Customer GSTIN: ' + (billing.gstin ? billing.gstin : 'Not Provided / B2C'), size: 10, gap: 30 },
    // Item & pricing table
    { text: 'Description', size: 11, gap: 18 },
    { text: finalPlanTitle, size: 11, gap: 20 },
    { text: 'Base Price: ' + currencyLabel + baseAmount.toFixed(2), size: 10, gap: 16 },
    { text: '18% GST (Included): ' + currencyLabel + taxAmount.toFixed(2), size: 10, gap: 16 },
    { text: 'Total Amount: ' + currencyLabel + totalPaid.toFixed(2) + '*', size: 13, gap: 34 },
    // Footer
    { text: 'Thank you.', size: 12, center: true, gap: 20 },
    { text: seller.companyName + ', ' + seller.address, size: 9, center: true, gap: 14 },
    { text: 'Official Email: hello@getreadyjob.com | Site: www.getreadyjob.com', size: 9, center: true, gap: 20 },
    { text: '* All prices are inclusive of 18% GST where applicable.', size: 8, center: true, gap: 12 },
    { text: '* This is a computer-generated invoice and does not require a physical signature.', size: 8, center: true, gap: 12 }
  ];

  var content = '';
  var cursorY = 740;
  receiptLines.forEach(function (line) {
    var x = line.center ? Math.max(20, (pageWidth - estimatePdfTextWidth(line.text, line.size)) / 2) : 50;
    content += 'BT /F1 ' + line.size + ' Tf ' + x.toFixed(2) + ' ' + cursorY.toFixed(2) + ' Td (' + escapePdfText(line.text) + ') Tj ET\n';
    cursorY -= line.gap;
  });
  var stream = '<< /Length 0 >>\nstream\n' + content + 'endstream';
  var objects = [];
...
function resolveTaxBreakdown(totalPaidAmount, billing, currency) {
  ...
  // Plan rates are GST-inclusive: decompose the amount actually charged, never add tax on top.
  var baseAmount = isDomestic ? Number((totalPaid / 1.18).toFixed(2)) : totalPaid;
  ...
  return {
    ...
    gstRate: isDomestic ? 0.18 : 0,
    gstAmount: gstAmount,
    totalAmount: totalPaid,
    cgstAmount: isDomestic ? Number((gstAmount / 2).toFixed(2)) : 0,
    sgstAmount: isDomestic ? Number((gstAmount / 2).toFixed(2)) : 0,
    igstAmount: 0
  };
}
...
function sendPurchaseEmails(transaction, recipientEmail) {
  ...
  dispatchEmail({
    to: email,
    subject: 'Welcome to Get Ready Job – Your ' + transaction.planName + ' is Active! \uD83D\uDE80',
    text: 'Your subscription is active. Plan: ' + transaction.planName + '. Quota: Lifetime / Unlimited. Visit the dashboard to manage your account.'
  });
  dispatchEmail({
    to: email,
...
app.post('/api/create-payment-link', function (req, res) {
  ...
  var plan = getPlanById(planId);
  var planName = resolvePlanTitle(planId, plan ? plan.name : (req.body.planName || req.body.planTitle));
...
app.post('/api/razorpay-webhook', function (req, res) {
  ...
  try {
    ...
    if (!transaction) {
      ...
      var plan = getPlanById(planId);
      var planName = (pendingOrder && pendingOrder.planName) || resolvePlanTitle(planId, plan ? plan.name : notes.plan_name);
