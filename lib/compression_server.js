...
function buildInvoicePdfBuffer(transaction) {
  ...
  addText('Get Ready Job - Tax Invoice / Receipt', 14, { align: 'center', gap: 22 });
  addText('Thanks for your purchase! For any questions, contact hello@getreadyjob.com', 9, { align: 'center', gap: 46 });
  ...
  addText('Customer GSTIN: ' + (billing.gstin ? billing.gstin : 'Not Provided / B2C'), 10, { gap: 50 });
  ...
  addRow('18% GST (Included)', currencyLabel + taxAmount.toFixed(2), 10, 22);
  addRule(26);
...
// Sequential audit numbering: GRJ/YYYY-MM/0001, resets each calendar month, never reused or skipped.
function generateNextInvoiceNumber(referenceDate) {
  ...
  var nextIndex = Number(state[yearMonth] || 0) + 1;
  state[yearMonth] = nextIndex;
  persistInvoiceCounterState(state);
  return 'GRJ/' + yearMonth + '/' + String(nextIndex).padStart(4, '0');
}
