...
function getReportRowFromTransaction(transaction) {
  ...
  var sezStatus = String((transaction && (transaction.sezStatus || transaction.sez || (billing && (billing.sezStatus || billing.sez)))) || 'NO').trim().toUpperCase();
...
app.post('/api/razorpay-webhook', function (req, res) {
  ...
  try {
    ...
    if (!transaction) {
      ...
      var planId = String(notes.plan_id || (pendingOrder ? pendingOrder.planId : 'lifetime-pro') || 'lifetime-pro').trim();
      var plan = getPlanById(planId);
      var planName = resolvePlanTitle(planId, (pendingOrder && pendingOrder.planName) || (plan ? plan.name : notes.plan_name), Number(amountPaise || 0) / 100);
      ...
      var userAccount = upsertUserAccount({
        ...
        planName: planName,
        planStatus: 'active',
        accessExpiresAt: getPlanAccessExpiry(planId, planName, Number(amountPaise || 0) / 100)
      });
      userAccount.planStatus = 'active';
      userAccount.planId = planId;
      userAccount.planName = planName;
      userAccount.accessExpiresAt = getPlanAccessExpiry(planId, planName, Number(amountPaise || 0) / 100);
      userAccount.updatedAt = new Date().toISOString();

      logAuditEvent(recipientEmail || 'guest', 'payment-webhook-captured', { transactionId: transaction.transactionId, orderId: orderId, paymentId: paymentId, eventType: eventType, gstin: billing.gstin || '', state: billing.state || '' });
