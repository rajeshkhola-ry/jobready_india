...
var EXPORT_TRANSACTION_TYPE = 'Export (Zero-Rated)';
...
function buildInvoicePdfBuffer(transaction) {
  ...
  var invoiceTitleText = isAmendedInvoice ? 'AMENDED / REVISED TAX INVOICE' : 'TAX INVOICE / RECEIPT';

  // Header & branding
  addText('GET READY JOB', 22, { align: 'center', gap: 18 });
  addText(invoiceTitleText, 12, { align: 'left', gap: 18 });
  ...
  addText('Customer GSTIN: ' + (isExportInvoice ? 'Not Applicable (Export of Services)' : (billing.gstin ? billing.gstin : 'Not Provided / B2C')), 10, { gap: 22 });
  addText('Customer Country: ' + (billing.country ? billing.country : 'India'), 10, { gap: 20 });
  if (!isExportInvoice) {
    addText('Customer State: ' + (billing.state ? billing.state : 'India'), 10, { gap: 20 });
  }
  addText('Place of Supply (POS): ' + (isExportInvoice
    ? (EXPORT_POS_CODE + '-' + EXPORT_POS_STATE + ' (Export Out of India)')
    : ((posContext.state || sellerGstProfile.stateName) + ' (' + (posContext.stateCode || sellerGstProfile.stateCode) + ')')), 10, { gap: 20 });
  ...
  addText(isExportInvoice
    ? '* Export of services - zero-rated supply. No GST charged (Place of Supply: 96-Other Countries).'
    : '* All prices are inclusive of 18% GST where applicable.', 8, { align: 'center', gap: 16 });
  addText('* This is a computer-generated invoice and does not require a physical signature.', 8, { align: 'center', gap: 16 });

  var content = '0.12 w\n0.22 0.40 RG\n' + marginLeft + ' ' + marginLeft + ' m ' + marginRight + ' ' + marginLeft + ' l ' + marginRight + ' ' + (792 - marginLeft) + ' l ' + marginLeft + ' ' + (792 - marginLeft) + ' l h S\n0 0 0 RG\n';
  var cursorY = 740;
...
function buildPurchaseWelcomeEmail(transaction) {
...
function sendPurchaseEmails(transaction, recipientEmail) {
  ...
  var mail = buildPurchaseWelcomeEmail(transaction);
  dispatchEmail({
    to: email,
...
app.post('/api/admin/email/send-sample-invoice', requireAdmin, function (req, res) {
  var payload = req.body || {};
  ...
  var mail = buildPurchaseWelcomeEmail(sampleTransaction);
  dispatchEmail({
    to: to,
    subject: mail.subject,
    text: mail.text,
...
app.get('/api/user/account', function (req, res) {
  ...
  if (!user) {
    ...
    user = upsertUserAccount({
      ...
      planStatus: 'active',
      accessExpiresAt: getPlanAccessExpiry(latest.planId, latest.planName, latest.totalAmount || latest.amount)
    });
  }
