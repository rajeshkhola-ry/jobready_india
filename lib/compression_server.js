...
// entitlement always matches whichever tier the customer actually paid for. Numbers match
// what the dashboard has always shown users (10/60/180/500), so the balance a customer
// sees does not suddenly change from a familiar figure to an unfamiliar one.
function getQuotaEntitlementForPlan(planId, planName, amountInRupees) {
  var amount = Number(amountInRupees || 0);
  var normalizedPlanId = String(planId || '').trim().toLowerCase();
  if (normalizedPlanId === 'lifetime-pro') {
    return { total: 500, isUnlimited: false };
  }
  if (amount === 999 || normalizedPlanId === 'yearly') {
