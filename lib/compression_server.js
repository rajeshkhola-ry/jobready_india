...
var ADMIN_CONFIG = {
  email: process.env.ADMIN_EMAIL || 'admin@getreadyjob.com',
...
function createTwoFactorChallenge(email) {
...
app.post(['/api/admin/login', '/admin/login', '/admin/login/'], function (req, res) {
  ...
  if (ADMIN_CONFIG.twoFactorEnabled && ADMIN_CONFIG.twoFactorSecret) {
    var challengeToken = createTwoFactorChallenge(email);
    return res.json({
      success: true,
      requires2fa: true,
  ...
  var token = adminAuth.createAdminToken({ email: email, role: 'admin', exp: expiry }, ADMIN_CONFIG.jwtSecret);
  res.json({ success: true, requires2fa: false, token: token, role: 'admin', expiresAt: expiry });
});
