...
function tryLocalIntentMatch(transcriptRaw) {
  ...
  if (/\bconvert\b/.test(transcript)) {
    ...
    var targetMentionsExcelCsv = /\b(excel|xlsx|csv|spreadsheet)\b/.test(targetPart);
...
app.post('/api/voice-command', voiceCommandUpload.single('audio'), function (req, res) {
  ...
  var localMatch = transcriptHint ? tryLocalIntentMatch(transcriptHint) : null;
  if (localMatch) {
