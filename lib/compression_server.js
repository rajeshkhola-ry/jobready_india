...
// Permissive CORS for every route (including file upload/download endpoints) -
// must be registered before any route/body-parser so preflight OPTIONS
// requests are answered here and never reach multer/auth middleware.
var corsOptions = {
  origin: ['https://getreadyjob.com', 'https://www.getreadyjob.com', 'http://localhost:54341', 'http://localhost:54340'],
  methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'],
  allowedHeaders: ['Content-Type', 'Authorization', 'Accept', 'Origin', 'X-Requested-With'],
  exposedHeaders: ['Content-Disposition', 'Content-Length', 'Content-Type', 'X-Conversion-Engine'],
  credentials: false
};
...
function resolvePlanTitle(planId, rawPlanName, totalPaid) {
  ...
  // Key checks cover both the live client's plan ids ('7days'/'monthly'/'yearly'/'lifetime')
  // and the legacy admin-catalog ids ('weekly-pass'/'pro-monthly'/'lifetime-pro').
  if (amount === 199 || key === '7days' || key === '7-day' || key === 'short-access' || key === 'weekly-pass') {
    return '7 Days Access';
  }
  if (amount === 499 || key === 'monthly' || key === 'pro-monthly') {
    return '1 Month Pro';
  }
  if (amount === 2499 || key === 'yearly') {
    return '1 Year Unlimited Access';
  }
  if (amount === 19999 || key === 'lifetime' || key === 'lifetime-pro') {
    return 'Lifetime Pro';
  }
...
function getPlanAccessExpiry(planId, planName, amountInRupees) {
  ...
  if (amount === 199 || normalizedPlanId === '7days' || normalizedPlanId === 'weekly-pass' || normalizedPlanId === '7-day' || normalizedPlanId === 'short-access') {
    return new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
  }
  if (amount === 499 || normalizedPlanId === 'pro-monthly' || normalizedPlanId === 'monthly') {
    return new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString();
  }
  if (amount === 2499 || normalizedPlanId === 'yearly') {
    return new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString();
  }
  if (amount === 19999 || normalizedPlanId === 'lifetime' || normalizedPlanId === 'lifetime-pro') {
...
function getQuotaEntitlementForPlan(planId, planName, amountInRupees) {
  ...
  if (amount === 199 || normalizedPlanId === 'weekly-pass' || normalizedPlanId === '7-day' || normalizedPlanId === 'short-access' || normalizedPlanId === '7days') {
...
var defaultPlanCatalogConfig = {
  inr_prices: { Free: 0, '7Days': 199, Monthly: 499, Yearly: 2499, Lifetime: 19999 },
  usd_prices: { Free: 0, '7Days': 4.99, Monthly: 9.99, Yearly: 29.99, Lifetime: 199.99 },
  enabled_tools_by_plan: {},
  user_quotas_by_plan: {},
  voice_quotas_by_plan: {},
  ocr_quotas_by_plan: {}
};
...
// Explicit existence check (not a truthy check) so an admin-configured "0"
// quota (zero voice commands / zero user quota) is preserved exactly -
// `value || fallback`-style checks would incorrectly treat 0 as "missing".
function sanitizePlanCatalogQuotas(rawQuotas, fallback) {
  var out = Object.assign({}, fallback);
  if (rawQuotas && typeof rawQuotas === 'object') {
    Object.keys(rawQuotas).forEach(function (planKey) {
      var value = rawQuotas[planKey];
      if (value !== null && value !== undefined && String(value).trim() !== '') {
        out[planKey] = String(value).trim();
      }
    });
  }
  return out;
...
function readPlanCatalogState() {
  try {
    ...
    return {
      ...
      user_quotas_by_plan: sanitizePlanCatalogQuotas(parsed.user_quotas_by_plan, {}),
      voice_quotas_by_plan: sanitizePlanCatalogQuotas(parsed.voice_quotas_by_plan, {}),
      ocr_quotas_by_plan: sanitizePlanCatalogQuotas(parsed.ocr_quotas_by_plan, {})
    };
  } catch (err) {
...
// Voice top-up credits are additive (never reset) - mirrors upsertUserAccount's
// allocate-on-purchase-only pattern but for the separate voice-commands balance.
function creditVoiceCommands(email, credits) {
  var normalizedEmail = String(email || '').trim().toLowerCase();
  var user = userAccounts.find(function (item) {
    return normalizedEmail && item.email && item.email.toLowerCase() === normalizedEmail;
  });
  if (!user) {
    user = upsertUserAccount({ email: normalizedEmail });
  }
  var creditAmount = Number(credits || 0);
  user.voiceCommandsBalance = Number(user.voiceCommandsBalance || 0) + creditAmount;
  user.voiceCommandsTotal = Number(user.voiceCommandsTotal || 0) + creditAmount;
  user.updatedAt = new Date().toISOString();
  persistUserAccountState();
  return user;
}
...
// ---------------------------------------------------------------------------
// Official GSTR-1 multi-tab Excel exporter (GST Offline Tool schema:
// b2cs / b2b / hsn / doc_issue sheets), built on top of the same
// buildSalesReportData() rows used by the existing CSV export above.
// ---------------------------------------------------------------------------
function formatPlaceOfSupplyLabel(row) {
  var stateName = String(row.placeOfSupply || '').split(' (')[0].trim() || sellerGstProfile.stateName;
  var stateCode = String(row.placeOfSupplyCode || '').trim() || sellerGstProfile.stateCode;
  return stateCode + '-' + stateName;
}

function resolveRowGstRatePercent(row) {
  var igstRate = Number(row.igstRate || 0);
  if (igstRate > 0) {
    return igstRate;
  }
  var splitRate = Number(row.cgstRate || 0) + Number(row.sgstRate || 0);
  return splitRate > 0 ? splitRate : Number(invoiceTaxConfig.domesticGstRate || 0.18) * 100;
}

// b2cs: B2C Small/Others - aggregated per Place of Supply + Rate (no invoice-level
// detail required for B2CS under GSTR-1 rules).
function buildB2csRows(reportRows) {
  var aggregates = {};

  (reportRows || []).forEach(function (row) {
    if (row.documentType !== 'TAX_INVOICE' || row.transactionType !== 'B2C') {
      return;
    }
    var ratePercent = resolveRowGstRatePercent(row);
    var key = row.placeOfSupplyCode + '|' + ratePercent;
    if (!aggregates[key]) {
      aggregates[key] = {
        type: 'OE',
        placeOfSupply: formatPlaceOfSupplyLabel(row),
        rate: Number(ratePercent.toFixed(1)),
        taxableValue: 0,
        cessAmount: 0,
        ecommerceGstin: ''
      };
    }
    aggregates[key].taxableValue += Number(row.taxableValue || 0);
  });

  return Object.keys(aggregates).map(function (key) {
    var item = aggregates[key];
    item.taxableValue = Number(item.taxableValue.toFixed(2));
    return item;
  });
}

// b2b: B2B Registered Users - one row per invoice with a valid recipient GSTIN
// (invoice-level detail is required for B2B, unlike the aggregated B2CS sheet).
function buildB2bRows(reportRows) {
  return (reportRows || [])
    .filter(function (row) {
      return row.documentType === 'TAX_INVOICE' && row.transactionType === 'B2B' && row.customerGstin;
    })
    .map(function (row) {
      return {
        gstin: row.customerGstin,
        receiverName: row.customerName || '',
        invoiceNumber: row.invoiceNumber || row.documentNumber || '',
        invoiceDate: row.invoiceDate || '',
        invoiceValue: Number(row.totalInvoiceAmount || 0),
        placeOfSupply: formatPlaceOfSupplyLabel(row),
        reverseCharge: 'N',
        applicableRatePercent: '',
        invoiceType: 'Regular',
        ecommerceGstin: '',
        rate: Number(resolveRowGstRatePercent(row).toFixed(1)),
...
// hsn: HSN Summary of Outward Supplies - this business issues a single service
// (SAC 998313) so all invoices roll up into one summary row.
function buildHsnSummaryRows(reportRows) {
  var invoiceRows = (reportRows || []).filter(function (row) { return row.documentType === 'TAX_INVOICE'; });
  if (!invoiceRows.length) {
    return [];
  }

  var totals = invoiceRows.reduce(function (acc, row) {
    acc.totalValue += Number(row.totalInvoiceAmount || 0);
    ...
    return acc;
  }, { totalValue: 0, taxableValue: 0, igst: 0, cgst: 0, sgst: 0 });

  return [{
    hsnCode: invoiceSellerProfile.sacCode || '998313',
    description: 'Information technology (IT) design and development services',
    uqc: 'OTH',
    totalQuantity: invoiceRows.length,
    totalValue: Number(totals.totalValue.toFixed(2)),
    taxableValue: Number(totals.taxableValue.toFixed(2)),
    integratedTax: Number(totals.igst.toFixed(2)),
    centralTax: Number(totals.cgst.toFixed(2)),
    stateTax: Number(totals.sgst.toFixed(2)),
    cess: 0
  }];
}

function parseDocumentSequenceNumber(documentNumber) {
  var match = /\/(\d+)$/.exec(String(documentNumber || '').trim());
  return match ? parseInt(match[1], 10) : null;
}

// doc_issue: Documents Issued during the tax period - one row per document series
// (tax invoices, credit notes, debit notes), including cancelled invoices, which
// buildSalesReportData() deliberately excludes from the revenue-facing rows above.
function buildDocIssueRows(range, fromDate, toDate) {
  var bounds = getDateRangeBounds(range, fromDate, toDate);

  function withinBounds(dateValue) {
    var ts = new Date(dateValue || Date.now());
    if (isNaN(ts.getTime())) {
      return false;
    }
    if (bounds.start && ts < bounds.start) {
  ...
  var invoiceTransactions = salesTransactions.filter(function (item) {
    return item && item.invoiceNumber && withinBounds(item.createdAt || item.paidAt);
  });
  var creditNotes = salesCreditDebitNotes.filter(function (item) {
    return item && item.documentType === 'CREDIT_NOTE' && withinBounds(item.createdAt);
  });
  var debitNotes = salesCreditDebitNotes.filter(function (item) {
    return item && item.documentType === 'DEBIT_NOTE' && withinBounds(item.createdAt);
  });

  function summarizeSeries(natureOfDocument, documentNumbers, cancelledCount) {
    var sequenceNumbers = documentNumbers
      .map(parseDocumentSequenceNumber)
      .filter(function (value) { return value !== null; })
      .sort(function (a, b) { return a - b; });

    var total = sequenceNumbers.length;
    var cancelled = Math.min(cancelledCount || 0, total);
    return {
      natureOfDocument: natureOfDocument,
      srNoFrom: total ? sequenceNumbers[0] : 0,
      srNoTo: total ? sequenceNumbers[total - 1] : 0,
      totalNumber: total,
      cancelled: cancelled,
  ...
  var cancelledInvoiceCount = invoiceTransactions.filter(function (item) {
    return String(item.status || '').toLowerCase() === 'cancelled';
  }).length;

  return [
    summarizeSeries('Invoices for outward supply', invoiceTransactions.map(function (item) { return item.invoiceNumber; }), cancelledInvoiceCount),
    summarizeSeries('Credit Note', creditNotes.map(function (item) { return item.documentNumber; }), 0),
    summarizeSeries('Debit Note', debitNotes.map(function (item) { return item.documentNumber; }), 0)
  ].filter(function (row) { return row.totalNumber > 0; });
}

function buildGstr1ExcelWorkbook(report) {
  var workbook = new ExcelJS.Workbook();
  workbook.creator = 'GetReadyJob';
  workbook.created = new Date();

  var b2csSheet = workbook.addWorksheet('b2cs');
  b2csSheet.columns = [
    { header: 'Type', key: 'type', width: 8 },
    { header: 'Place Of Supply', key: 'placeOfSupply', width: 20 },
    { header: 'Applicable % of Tax Rate', key: 'rate', width: 22 },
  ...
  buildB2csRows(report.rows).forEach(function (row) { b2csSheet.addRow(row); });
  b2csSheet.getRow(1).font = { bold: true };

  var b2bSheet = workbook.addWorksheet('b2b');
  b2bSheet.columns = [
    { header: 'GSTIN/UIN of Recipient', key: 'gstin', width: 18 },
    { header: 'Receiver Name', key: 'receiverName', width: 22 },
    { header: 'Invoice Number', key: 'invoiceNumber', width: 20 },
    { header: 'Invoice date', key: 'invoiceDate', width: 14 },
    { header: 'Invoice Value', key: 'invoiceValue', width: 14 },
    { header: 'Place Of Supply', key: 'placeOfSupply', width: 20 },
    { header: 'Reverse Charge', key: 'reverseCharge', width: 14 },
    { header: 'Applicable % of Tax Rate', key: 'applicableRatePercent', width: 22 },
  ...
  buildB2bRows(report.rows).forEach(function (row) { b2bSheet.addRow(row); });
  b2bSheet.getRow(1).font = { bold: true };

  var hsnSheet = workbook.addWorksheet('hsn');
  hsnSheet.columns = [
    ...
    { header: 'Integrated Tax', key: 'integratedTax', width: 14 },
    { header: 'Central Tax', key: 'centralTax', width: 14 },
    { header: 'State/UT Tax', key: 'stateTax', width: 14 },
    { header: 'Cess', key: 'cess', width: 10 }
  ];
  buildHsnSummaryRows(report.rows).forEach(function (row) { hsnSheet.addRow(row); });
  hsnSheet.getRow(1).font = { bold: true };

  var docIssueSheet = workbook.addWorksheet('doc_issue');
  docIssueSheet.columns = [
    { header: 'Nature of Document', key: 'natureOfDocument', width: 32 },
    { header: 'Sr. No. From', key: 'srNoFrom', width: 14 },
    { header: 'Sr. No. To', key: 'srNoTo', width: 14 },
  ...
  buildDocIssueRows(report.range, report.fromDate, report.toDate).forEach(function (row) { docIssueSheet.addRow(row); });
  docIssueSheet.getRow(1).font = { bold: true };
...
// ---------------------------------------------------------------------------
// Resolves the real MIME type for an upload, falling back to the file
// extension whenever the browser/OS reports a generic/ambiguous type such as
// application/octet-stream (common for XLSX and some PDF sources) - this is
// what actually decides whether a valid file gets rejected with a 400.
// ---------------------------------------------------------------------------
var UPLOAD_EXTENSION_MIME_MAP = {
  '.pdf': 'application/pdf',
  '.jpg': 'image/jpeg',
  '.jpeg': 'image/jpeg',
  '.png': 'image/png',
...
var GENERIC_UPLOAD_MIME_TYPES = ['application/octet-stream', 'application/binary', 'application/download', ''];

function resolveUploadMimeType(file) {
  var raw = ((file && file.mimetype) || '').split(';')[0].trim().toLowerCase();
  if (raw && GENERIC_UPLOAD_MIME_TYPES.indexOf(raw) === -1) {
    return raw;
  }
  var ext = path.extname((file && file.originalname) || '').toLowerCase();
  return UPLOAD_EXTENSION_MIME_MAP[ext] || raw || 'application/octet-stream';
}

// ---------------------------------------------------------------------------
// Multer configuration (100 MB limit, PDF + image + spreadsheet uploads)
// ---------------------------------------------------------------------------
var upload = multer({
  dest: UPLOAD_DIR,
  ...
  fileFilter: function (req, file, cb) {
    ...
    var resolved = resolveUploadMimeType(file);
    if (allowed.indexOf(resolved) !== -1) {
      file.resolvedMimeType = resolved;
      cb(null, true);
    } else {
      cb(new Error('Invalid file type: ' + (file.mimetype || 'unknown') + ' (' + (file.originalname || 'file') + ')'));
    }
  }
});
...
// Resolves the ImageMagick CLI binary used for the lightweight deskew/
// auto-contrast pass on rasterized PDF pages (see deskewAndEnhanceImage).
function resolveImageMagickBinary() {
  ...
  if (process.platform === 'win32') {
    candidates.push('magick', 'convert');
  }
...
// ---------------------------------------------------------------------------
// Multi-pass target-size PDF compression.
// Pass 1: standard downsampling (/ebook, 150 DPI). Pass 2: aggressive
// downsampling (/screen, 72-96 DPI, JPEG 45-60%). Pass 3: further downsample
// plus font-subset/object-stream flattening for a final squeeze. Always
// returns the smallest candidate found across every pass; never silently
// returns the original bytes when a target size was requested.
// Smart lossless-visual: 1-bit monochrome image streams (typical of scanned
// text pages) are NEVER downsampled, so text stays crisp/unblurred - only
// color/grayscale raster images are downsampled to hit the target size.
// Output is linearized (-dFastWebView) for fast progressive-download streaming.
// ---------------------------------------------------------------------------
function compressPdfWithGhostscriptSettings(inputPath, outputPath, options, callback) {
  var execFile = require('child_process').execFile;
  var gsBin = resolveGhostscriptBinary();
  // Safety floor: never request a resolution below 72 DPI, even for an
  // aggressive target size - going lower is what previously produced
  // blank/unreadable pages when re-compressing an already-compressed PDF.
  var MIN_SAFE_DPI = 72;
  var safeDpi = Math.max(MIN_SAFE_DPI, options.dpi);
  var gsArgs = [
    '-dSAFER',
    '-dBATCH',
    '-dNOPAUSE',
    ...
    '-dColorImageDownsampleType=/Bicubic',
    '-dGrayImageDownsampleType=/Bicubic',
    '-dColorImageResolution=' + safeDpi,
    '-dGrayImageResolution=' + safeDpi,
    '-dMonoImageResolution=' + MIN_SAFE_DPI,
  ...
  execFile(gsBin, gsArgs, { maxBuffer: 20 * 1024 * 1024, timeout: 120000 }, function (err) {
    if (err) {
      return callback(new Error('Ghostscript pass (' + options.pdfSettings + ') failed: ' + err.message));
    }
    if (fileSize(outputPath) === 0) {
      return callback(new Error('Ghostscript pass (' + options.pdfSettings + ') produced empty output'));
    }
    callback(null);
  });
}
...
// ---------------------------------------------------------------------------
// Safe-output validation: rasterizes page 1 of a compression candidate at a
// low DPI and checks whether it is suspiciously uniform (near-white, almost
// no variation) - the signature of a page whose image content was stripped
// by overly aggressive downsampling. Fails OPEN (reports "not blank") on any
// rendering/analysis error, so a validation hiccup never blocks an otherwise
// good candidate.
// ---------------------------------------------------------------------------
function isPdfPageBlank(pdfPath, callback) {
  var execFile = require('child_process').execFile;
  var gsCommand = resolveGhostscriptBinary();
  var previewPath = pdfPath + '.blankcheck_' + Date.now() + '_' + Math.round(Math.random() * 1e6) + '.png';
  ...
  execFile(gsCommand, gsArgs, { timeout: 20000, maxBuffer: 20 * 1024 * 1024 }, function (gsErr) {
    if (gsErr || !fs.existsSync(previewPath) || fileSize(previewPath) === 0) {
    ...
    sharp(previewPath).stats().then(function (stats) {
      cleanupFile(previewPath);
      var channels = (stats && stats.channels) || [];
      var isBlank = channels.length > 0 && channels.every(function (channel) {
        return channel.mean >= 250 && channel.stdev <= 4;
      });
      callback(null, isBlank);
    }).catch(function () {
      cleanupFile(previewPath);
      callback(null, false);
    });
...
function compressPdfToTarget(inputPath, outputPath, targetBytes, callback) {
  var PDFDocument;
  try { PDFDocument = require('pdf-lib').PDFDocument; } catch (e) { PDFDocument = null; }

  var origSize = fileSize(inputPath);
  if (origSize === 0) return callback(new Error('PDF file is empty'));

  var stamp = Date.now() + '_' + Math.round(Math.random() * 1e6);
  var pass0Path = outputPath + '.p0_' + stamp + '.pdf';
  var pass1Path = outputPath + '.p1_' + stamp + '.pdf';
  var pass2Path = outputPath + '.p2_' + stamp + '.pdf';
  var pass3Path = outputPath + '.p3_' + stamp + '.pdf';
  var allTempPaths = [pass0Path, pass1Path, pass2Path, pass3Path];

  function cleanupTemps(keepPath) {
    allTempPaths.forEach(function (p) {
      if (p !== keepPath) cleanupFile(p);
    });
  }

  // best.path may be inputPath itself until a pass actually beats the original.
  var best = { path: inputPath, size: origSize, mode: 'original', note: '' };

  function considerCandidate(candidatePath, mode, note) {
    var size = fileSize(candidatePath);
    if (size > 0 && size < best.size) {
      best = { path: candidatePath, size: size, mode: mode, note: note };
      return true;
    }
    return false;
  }
  ...
  function targetMet() {
    return !targetBytes || best.size <= targetBytes;
  }

  function finish() {
    if (targetBytes && best.path === inputPath) {
      cleanupTemps(null);
      return callback(new Error('Unable to compress this PDF below the requested target size with the available compression engine.'));
    }

    if (best.path === inputPath) {
      cleanupTemps(null);
      return callback(null, {
        ...
        ratio: '0',
        mode: 'original',
        note: 'PDF is already optimized; no further reduction was possible.',
        targetMet: targetBytes ? origSize <= targetBytes : null
      });
    }
    ...
    try {
      ...
      cleanupTemps(null);
      return callback(new Error('Cannot persist compressed output: ' + copyErr.message));
    }

    cleanupTemps(null);
    var finalSize = fileSize(outputPath);
    var reduction = origSize > 0 ? (((origSize - finalSize) / origSize) * 100).toFixed(1) : '0';
    callback(null, {
      success: true,
      originalSize: origSize,
      ...
      mode: best.mode,
      note: best.note,
      targetMet: targetBytes ? finalSize <= targetBytes : null
    });
  }

  function runPass0StructureOptimize(next) {
    if (!PDFDocument) return next();
    var pdfBytes;
    try {
    ...
    PDFDocument.load(pdfBytes).then(function (doc) {
      return doc.save({ useObjectStreams: true, addDefaultPage: false });
    }).then(function (optimized) {
      if (optimized && optimized.length > 0) {
        fs.writeFileSync(pass0Path, optimized);
        considerCandidate(pass0Path, 'structure-optimized', 'PDF structure optimized (object streams, no rasterization).');
      }
      next();
    }).catch(function () {
      next();
  ...
  function runPass1(next) {
    compressPdfWithGhostscriptSettings(inputPath, pass1Path, { pdfSettings: '/ebook', dpi: 150, jpegQuality: 75 }, function (err) {
      if (err) return next();
      isPdfPageBlank(pass1Path, function (_checkErr, blank) {
        if (!blank) {
          considerCandidate(pass1Path, 'standard-ebook', 'Standard downsampling applied (150 DPI, /ebook preset).');
        }
        next();
      });
    });
  ...
  function runPass2(next) {
    compressPdfWithGhostscriptSettings(inputPath, pass2Path, { pdfSettings: '/screen', dpi: 85, jpegQuality: 52 }, function (err) {
      if (err) return next();
      isPdfPageBlank(pass2Path, function (_checkErr, blank) {
        if (!blank) {
          considerCandidate(pass2Path, 'aggressive-screen', 'Aggressive downsampling applied (85 DPI, /screen preset, JPEG quality 52%).');
        }
        next();
      });
    });
  ...
  function runPass3(next) {
    // Feed the current best candidate back into Ghostscript for a final, more
    // aggressive squeeze - flattens redundant XObjects/fonts on re-write.
    var sourceForPass3 = best.path;
    compressPdfWithGhostscriptSettings(sourceForPass3, pass3Path, { pdfSettings: '/screen', dpi: 72, jpegQuality: 45 }, function (err) {
      if (err) return next();
      isPdfPageBlank(pass3Path, function (_checkErr, blank) {
        if (!blank) {
          considerCandidate(pass3Path, 'flatten-final', 'Final pass applied: 72 DPI, JPEG quality 45%, fonts subset and redundant objects flattened.');
        }
        next();
      });
    });
  ...
  runPass0StructureOptimize(function () {
    if (targetMet() && best.path !== inputPath) return finish();
    runPass1(function () {
      if (targetMet() && best.path !== inputPath) return finish();
      runPass2(function () {
        if (targetMet() && best.path !== inputPath) return finish();
...
// ---------------------------------------------------------------------------
// Photo HD Workspace - server-side preset rendering (sharp/libvips) for large
// canvases (A2/A3 posters, 2K+) that would otherwise block the browser's
// single UI thread for several seconds - Flutter Web's compute()/Isolate.run
// do not provide real background-thread execution on the standard (non-Wasm)
// web build, so heavy resize/encode work for these presets is offloaded here
// instead. Resizes (aspect-fit) onto an exact target canvas padded with the
// requested background color, optionally sharpens for "HD mode", and encodes
// to JPEG/PNG (with a quality/size shrink loop) or a print-ready PDF page.
// ---------------------------------------------------------------------------
function renderPhotoPresetWithSharp(inputPath, outputPath, options, callback) {
  var sharp;
  try { sharp = require('sharp'); } catch (e) {
    return callback(new Error('sharp module not available. Re-run npm install'));
  }

  var width = Math.max(1, parseInt(options.width, 10) || 0);
  var height = Math.max(1, parseInt(options.height, 10) || 0);
  if (!width || !height) {
    return callback(new Error('Invalid target width/height'));
  }

  var backgroundColor = options.backgroundColor || '#FFFFFF';
  var enableHdMode = !!options.enableHdMode;
  var outputFormat = (options.outputFormat || 'jpg').toLowerCase();
  var dpi = parseInt(options.dpi, 10) || 300;
  var maxTargetBytes = options.enforceFileSizeLimit
    ? Math.max(1, parseInt(options.maxTargetKb, 10) || 0) * 1024
    : null;

  function basePipeline() {
    var pipeline = sharp(inputPath).rotate().resize({
      width: width,
      height: height,
      fit: 'contain',
    ...
    if (enableHdMode) {
      pipeline = pipeline.modulate({ brightness: 1.01, saturation: 1.02 }).sharpen({ sigma: 0.8 });
    }
    return pipeline;
  }

  function shrinkToTargetJpeg(cb) {
    var quality = enableHdMode ? 98 : 94;

    function attempt() {
      basePipeline().jpeg({ quality: quality, progressive: true }).toBuffer(function (err, buffer) {
        if (err) return cb(err);
        if (!maxTargetBytes || buffer.length <= maxTargetBytes || quality <= 40) {
  ...
  function encodePngBuffer(cb) {
    basePipeline().png({ compressionLevel: 9 }).toBuffer(cb);
  }

  function buildPdfFromPng(pngBuffer, cb) {
    var PDFDocument;
    try { PDFDocument = require('pdf-lib').PDFDocument; } catch (e) {
      return cb(new Error('pdf-lib module not available. Re-run npm install'));
    }

    PDFDocument.create().then(function (pdfDoc) {
      return pdfDoc.embedPng(pngBuffer).then(function (pngImage) {
        var pageWidthPoints = (width / dpi) * 72;
        var pageHeightPoints = (height / dpi) * 72;
        var page = pdfDoc.addPage([pageWidthPoints, pageHeightPoints]);
        page.drawImage(pngImage, { x: 0, y: 0, width: pageWidthPoints, height: pageHeightPoints });
        return pdfDoc.save();
      });
    }).then(function (pdfBytes) {
      cb(null, Buffer.from(pdfBytes));
    }).catch(function (err) {
      cb(err);
    });
  }
  ...
  function finish(err, buffer) {
    ...
    fs.writeFile(outputPath, buffer, function (writeErr) {
      if (writeErr) return callback(writeErr);
      callback(null, { success: true, width: width, height: height });
    });
  }

  if (outputFormat === 'png') {
    encodePngBuffer(finish);
  } else if (outputFormat === 'pdf') {
    encodePngBuffer(function (err, pngBuffer) {
      if (err) return callback(err);
      buildPdfFromPng(pngBuffer, finish);
    });
  } else {
    shrinkToTargetJpeg(finish);
  }
}

// ---------------------------------------------------------------------------
// Lightweight background removal (classical color-distance "chroma key"
// cutout, no ML model). During development, both suggested ONNX-based
// options (@imgly/background-removal-node and the deprecated rembg-node)
// were verified hands-on to hold a permanent ~1GB resident model in memory
// once loaded - incompatible with this endpoint's <150MB budget and risky
// for the whole shared Render container, not just this one feature. This
// sharp-only approach estimates the background color from the image's own
// border pixels (works well for the app's dominant use case - ID/passport/
// product photos on a plain background), computes a per-pixel color-
// distance alpha mask with a soft feathered edge, and stays well under the
// RAM budget (a few tens of MB for typical photo sizes) - busy/complex
// backgrounds will cut out less precisely than a true AI segmentation model.
// ---------------------------------------------------------------------------
var BG_REMOVAL_MAX_DIMENSION = 3000;
var BG_REMOVAL_DISTANCE_THRESHOLD = 42;
var BG_REMOVAL_FEATHER_RANGE = 28;

function estimateBorderColor(data, width, height, channels) {
  var sumR = 0, sumG = 0, sumB = 0, count = 0;
  var borderThickness = Math.max(2, Math.round(Math.min(width, height) * 0.03));
  ...
  return {
    r: count ? Math.round(sumR / count) : 255,
    g: count ? Math.round(sumG / count) : 255,
    b: count ? Math.round(sumB / count) : 255
  };
}

function removeImageBackground(inputPath, outputPath, callback) {
  var sharp;
  try { sharp = require('sharp'); } catch (e) {
    return callback(new Error('sharp module not available. Re-run npm install'));
  }

  sharp(inputPath).rotate().metadata().then(function (meta) {
    var pipeline = sharp(inputPath).rotate();
    var longEdge = Math.max(meta.width || 0, meta.height || 0);
    if (longEdge > BG_REMOVAL_MAX_DIMENSION) {
      pipeline = pipeline.resize({
        width: BG_REMOVAL_MAX_DIMENSION,
        height: BG_REMOVAL_MAX_DIMENSION,
        fit: 'inside',
    ...
    var borderColor = estimateBorderColor(data, width, height, channels);
    var pixelCount = width * height;

    for (var i = 0; i < pixelCount; i++) {
      var offset = i * channels;
      var dr = data[offset] - borderColor.r;
      var dg = data[offset + 1] - borderColor.g;
      var db = data[offset + 2] - borderColor.b;
      var distance = Math.sqrt(dr * dr + dg * dg + db * db);

      var alpha;
      if (distance <= BG_REMOVAL_DISTANCE_THRESHOLD) {
        alpha = 0;
      } else if (distance >= BG_REMOVAL_DISTANCE_THRESHOLD + BG_REMOVAL_FEATHER_RANGE) {
        alpha = 255;
      } else {
        alpha = Math.round(((distance - BG_REMOVAL_DISTANCE_THRESHOLD) / BG_REMOVAL_FEATHER_RANGE) * 255);
      }
      data[offset + 3] = alpha;
    }

    return sharp(data, { raw: { width: width, height: height, channels: channels } })
      .blur(0.6)
      .png()
      .toFile(outputPath);
  }).then(function () {
    ...
    callback(new Error('Background removal failed: ' + (err && err.message ? err.message : err)));
  });
}
...
// ---------------------------------------------------------------------------
// Multi-pass target-size image compression (progressively lower quality and
// max dimension until the requested target is met). Mirrors
// compressPdfToTarget()'s "never silently return the original" guarantee.
// ---------------------------------------------------------------------------
function compressImageToTarget(inputPath, outputPath, format, targetBytes, callback) {
  var sharp;
  try { sharp = require('sharp'); } catch (e) {
    return callback(new Error('sharp module not available. Upgrade Node.js to v14+ and re-run npm install'));
  }
  ...
  var origSize = fileSize(inputPath);
  if (origSize === 0) return callback(new Error('Image file is empty'));

  var passes = [
    { quality: 82, maxDimension: 2400 },
    { quality: 68, maxDimension: 1920 },
    { quality: 55, maxDimension: 1600 },
    { quality: 42, maxDimension: 1200 },
    { quality: 32, maxDimension: 900 }
  ];

  var bestPath = null;
  var bestSize = origSize;
  var passIndex = 0;

  function finish() {
    if (!bestPath) {
      return callback(new Error('Unable to compress this image below the requested target size.'));
    }
    try {
      fs.copyFileSync(bestPath, outputPath);
    } catch (copyErr) {
      cleanupFile(bestPath);
      return callback(new Error('Cannot persist compressed output: ' + copyErr.message));
    }
    cleanupFile(bestPath);
    var finalSize = fileSize(outputPath);
    callback(null, {
      ...
      originalSize: origSize,
      compressedSize: finalSize,
      ratio: ((1 - finalSize / origSize) * 100).toFixed(2),
      targetMet: finalSize <= targetBytes
    });
  }
  ...
  function runNextPass() {
    ...
    passIndex++;
    var candidatePath = outputPath + '.c' + passIndex + '.img';

    var pipeline = sharp(inputPath).rotate().resize({
      width: pass.maxDimension,
      height: pass.maxDimension,
      fit: 'inside',
      withoutEnlargement: true
    });

    pipeline = format === 'webp'
      ? pipeline.webp({ quality: pass.quality })
      : pipeline.jpeg({ quality: pass.quality, progressive: true });

    pipeline.toFile(candidatePath, function (err) {
      if (!err) {
        var size = fileSize(candidatePath);
        if (size > 0 && size < bestSize) {
          ...
          cleanupFile(candidatePath);
        }
      }

      if (bestPath && bestSize <= targetBytes) {
        return finish();
      }
      runNextPass();
    });
...
// ---------------------------------------------------------------------------
// PDF -> DOCX high-fidelity conversion via headless LibreOffice.
// LibreOffice's PDF import + DOCX export reconstructs real editable
// paragraphs, native tables, fonts, and multi-column layout - far closer to
// the original visual appearance than image-embedding or plain text
// extraction. Each invocation gets its own isolated user-profile directory so
// concurrent conversions never collide, and a small in-process semaphore caps
// concurrency to protect server memory (LibreOffice is heavier than
// Ghostscript per invocation).
// ---------------------------------------------------------------------------
var LIBREOFFICE_MAX_CONCURRENT = parseInt(process.env.LIBREOFFICE_MAX_CONCURRENT, 10) || 2;
var libreOfficeActiveCount = 0;
var libreOfficeQueue = [];

function acquireLibreOfficeSlot(callback) {
  if (libreOfficeActiveCount < LIBREOFFICE_MAX_CONCURRENT) {
    libreOfficeActiveCount++;
    return callback();
  }
  libreOfficeQueue.push(callback);
}

function releaseLibreOfficeSlot() {
  if (libreOfficeQueue.length > 0) {
    var next = libreOfficeQueue.shift();
    next();
  } else {
    libreOfficeActiveCount = Math.max(0, libreOfficeActiveCount - 1);
  }
}

function removeWorkDir(dirPath) {
  try {
...
function resolveLibreOfficeBinary() {
  ...
  if (process.platform === 'win32') {
    ...
    // Absolute paths first - protects against "spawn soffice ENOENT" at
    // runtime even when a bare `soffice` lookup (or `which soffice` at
    // Docker build time, in a full shell environment) would normally
    // succeed. Covers both the /usr/bin wrapper script Debian/Ubuntu's
    // libreoffice-writer package installs and the real binary location.
    candidates.push(
      '/usr/bin/soffice',
...
// True if a LibreOffice (soffice) binary can actually be located on this
// host right now - used to fail fast with a clear, actionable error instead
// of surfacing a raw "spawn soffice ENOENT" from a doomed execFile() call.
function isLibreOfficeAvailable() {
  var resolved = resolveLibreOfficeBinary();
  if (resolved.indexOf(path.sep) !== -1 || resolved.indexOf(':') !== -1) {
    return fs.existsSync(resolved);
  }
  var pathDirs = (process.env.PATH || '').split(path.delimiter);
  var executableNames = process.platform === 'win32' ? [resolved + '.exe', resolved] : [resolved];
  for (var i = 0; i < pathDirs.length; i++) {
    if (!pathDirs[i]) continue;
    for (var j = 0; j < executableNames.length; j++) {
      if (fs.existsSync(path.join(pathDirs[i], executableNames[j]))) {
        return true;
      }
    }
  }
...
function convertPdfToDocxWithLibreOffice(inputPath, callback) {
  if (!isLibreOfficeAvailable()) {
    return callback(new Error('LibreOffice (soffice) is not installed or could not be found on this server - this fallback tier is unavailable.'));
  }

  var execFile = require('child_process').execFile;
  var sofficeBin = resolveLibreOfficeBinary();
  var jobId = 'lo_' + Date.now() + '_' + Math.round(Math.random() * 1e6);
  var profileDir = path.join(UPLOAD_DIR, 'profile_' + jobId);
  var outDir = path.join(UPLOAD_DIR, 'out_' + jobId);

  try {
    fs.mkdirSync(profileDir, { recursive: true });
    fs.mkdirSync(outDir, { recursive: true });
  } catch (dirErr) {
    return callback(new Error('Cannot create LibreOffice work directories: ' + dirErr.message));
  }

  acquireLibreOfficeSlot(function () {
    var args = [
      '--headless',
      '--norestore',
      '--invisible',
    ...
    execFile(sofficeBin, args, { timeout: 100000, maxBuffer: 20 * 1024 * 1024 }, function (err) {
      releaseLibreOfficeSlot();

      if (err) {
        removeWorkDir(profileDir);
        removeWorkDir(outDir);
        return callback(new Error('LibreOffice conversion failed: ' + err.message));
      }

      var baseName = path.parse(inputPath).name;
      var outputPath = path.join(outDir, baseName + '.docx');

      if (!fs.existsSync(outputPath) || fileSize(outputPath) === 0) {
        removeWorkDir(profileDir);
        removeWorkDir(outDir);
        return callback(new Error('LibreOffice did not produce a DOCX output for this PDF.'));
      }

      var docxBytes;
      try {
        docxBytes = fs.readFileSync(outputPath);
      } catch (readErr) {
        removeWorkDir(profileDir);
        removeWorkDir(outDir);
        return callback(new Error('Cannot read LibreOffice output: ' + readErr.message));
      }

      removeWorkDir(profileDir);
      removeWorkDir(outDir);
      callback(null, docxBytes);
    });
  });
}

// ---------------------------------------------------------------------------
// PDF -> DOCX primary engine: pdf2docx (Python, PyMuPDF-based). Detects real
// table bounding boxes, multi-column text flow, and embedded shapes far more
// accurately (~97-98%) than LibreOffice's PDF import filter. Tried first by
// convertPdfToDocxHighFidelity(); any failure (missing binary, non-zero exit,
// timeout, empty output) falls back to convertPdfToDocxWithLibreOffice above.
// ---------------------------------------------------------------------------
function resolvePdf2docxBinary() {
  if (process.env.PDF2DOCX_BIN) {
    return process.env.PDF2DOCX_BIN;
  }

  // Absolute paths first - same defensive pattern as resolveLibreOfficeBinary()/
  // resolvePython3Binary(), protects against a bare `pdf2docx` PATH lookup
  // failing at runtime (ENOENT) even when it would resolve fine in a full
  // Docker-build-time shell - the exact same class of bug that bit both
  // soffice and python3 in this file before. A root-level `pip3 install`
  // (this Dockerfile's install method) lands its console-script in
  // /usr/local/bin on Debian, checked first.
  var candidates = process.platform === 'win32'
    ? ['pdf2docx']
    : ['/usr/local/bin/pdf2docx', '/usr/bin/pdf2docx', 'pdf2docx'];

  for (var i = 0; i < candidates.length; i++) {
    var candidate = candidates[i];
    if (candidate.indexOf(path.sep) !== -1) {
...
// True if the pdf2docx console-script can actually be located on this host
// right now - mirrors isLibreOfficeAvailable()'s exact pattern. Diagnostics
// only, surfaced via /api/info + the startup log so "is the PRIMARY PDF-to-
// Word engine really resolvable on this server" can be checked without shell
// access - convertPdfToDocxWithPdf2docx still attempts the real conversion
// regardless and handles a genuinely missing binary via its own execFile
// error callback (falls through to Tier 2/LibreOffice).
function isPdf2docxAvailable() {
  var resolved = resolvePdf2docxBinary();
  if (resolved.indexOf(path.sep) !== -1 || resolved.indexOf(':') !== -1) {
    return fs.existsSync(resolved);
  }
  var pathDirs = (process.env.PATH || '').split(path.delimiter);
...
function convertPdfToDocxWithPdf2docx(inputPath, callback) {
  var execFile = require('child_process').execFile;
  var jobId = 'p2d_' + Date.now() + '_' + Math.round(Math.random() * 1e6);
  var outDir = path.join(UPLOAD_DIR, 'pdf2docx_' + jobId);

  try {
    fs.mkdirSync(outDir, { recursive: true });
  } catch (dirErr) {
    return callback(new Error('Cannot create pdf2docx work directory: ' + dirErr.message));
  }

  var outputPath = path.join(outDir, path.parse(inputPath).name + '.docx');
  var pdf2docxBin = resolvePdf2docxBinary();

  // Raised from 50s: complex, image/logo-heavy invoice-style PDFs (multi-
  // column layout analysis + embedded graphics) can genuinely need more time
  // than a simple text-only page - the outer route (340s) and client (350s)
  // timeouts still have ample headroom above this for the LibreOffice/Vision-
  // OCR fallback tiers if pdf2docx still can't finish.
  execFile(pdf2docxBin, ['convert', inputPath, outputPath], { timeout: 90000, maxBuffer: 20 * 1024 * 1024 }, function (err) {
    if (err) {
      removeWorkDir(outDir);
      return callback(new Error('pdf2docx conversion failed: ' + err.message));
    }

    if (!fs.existsSync(outputPath) || fileSize(outputPath) === 0) {
      removeWorkDir(outDir);
      return callback(new Error('pdf2docx did not produce a DOCX output for this PDF.'));
    }
    ...
    try {
      ...
      removeWorkDir(outDir);
      return callback(new Error('Cannot read pdf2docx output: ' + readErr.message));
    }
...
// ---------------------------------------------------------------------------
// Shared Google Cloud Vision OCR engine - powers BOTH the scanned-PDF -> Word
// path (/api/convert-scanned-pdf-to-docx) and the standalone Scanned PDF ->
// Searchable PDF tool (/api/ocr-pdf). Rasterizes pages via Ghostscript
// (existing, proven infra also used by the removed Tier 3), then batches ALL
// pages into a single Vision `images:annotate` (DOCUMENT_TEXT_DETECTION)
// request. A global monthly page cap (persisted to disk so it survives
// restarts) protects against runaway Vision API billing across every user/
// tool combined; per-plan monthly quotas are additionally enforced
// client-side (see lib/Services/ocr_quota_service.dart).
// ---------------------------------------------------------------------------
var OCR_GLOBAL_MONTHLY_PAGE_CAP = 990;
var OCR_RASTER_DPI = 200;
var ocrUsageStatePath = path.join(backupDir, 'ocr_usage_state.json');
var ocrGlobalUsage = null;
...
function loadOcrGlobalUsage() {
  if (ocrGlobalUsage) return ocrGlobalUsage;
  try {
    var raw = fs.readFileSync(ocrUsageStatePath, 'utf8');
    var parsed = JSON.parse(raw);
    ocrGlobalUsage = { monthKey: parsed.monthKey || currentMonthKey(), pagesUsed: Number(parsed.pagesUsed) || 0 };
  } catch (e) {
    ocrGlobalUsage = { monthKey: currentMonthKey(), pagesUsed: 0 };
  }
  return ocrGlobalUsage;
}

function saveOcrGlobalUsage() {
  try {
    fs.mkdirSync(path.dirname(ocrUsageStatePath), { recursive: true });
    fs.writeFileSync(ocrUsageStatePath, JSON.stringify(ocrGlobalUsage, null, 2));
  } catch (e) {
    console.error('[ocr] Could not persist global OCR usage state: ' + e.message);
  }
}

// Auto-resets pagesUsed to 0 the first time this is called in a new UTC
// calendar month - "resets on the 1st of next month" per the product spec.
function getOcrGlobalUsageState() {
  var state = loadOcrGlobalUsage();
...
// Reserves `pageCount` pages against the global monthly cap BEFORE any Vision
// API call is made, so an over-cap request is rejected safely instead of
// processed and discovered too late (wastes cost + Ghostscript/CPU work).
function reserveOcrGlobalPages(pageCount) {
  var state = getOcrGlobalUsageState();
  if (state.pagesUsed + pageCount > OCR_GLOBAL_MONTHLY_PAGE_CAP) {
    return { allowed: false, used: state.pagesUsed, cap: OCR_GLOBAL_MONTHLY_PAGE_CAP };
  }
  state.pagesUsed += pageCount;
  saveOcrGlobalUsage();
  return { allowed: true, used: state.pagesUsed, cap: OCR_GLOBAL_MONTHLY_PAGE_CAP };
}

function resolveGoogleVisionApiKey() {
  return process.env.GOOGLE_CLOUD_VISION_API_KEY || '';
}

// Rasterizes every page of a PDF to a JPEG (at OCR_RASTER_DPI) via
// Ghostscript, returning page image paths in page order plus their pixel
// dimensions (needed to convert Vision's pixel bounding boxes to PDF points
// later). Mirrors the same memory-bounded (-dMaxBitmap) rasterization
// already used for the scanned-PDF text pre-flight check.
function rasterizePdfPagesForOcr(inputPath, callback) {
  var execFile = require('child_process').execFile;
  var gsCommand = resolveGhostscriptBinary();
  var jobId = 'ocr_raster_' + Date.now() + '_' + Math.round(Math.random() * 1e6);
  var pageDir = path.join(UPLOAD_DIR, jobId);

  try {
    fs.mkdirSync(pageDir, { recursive: true });
  } catch (dirErr) {
    return callback(new Error('Cannot create OCR work directory: ' + dirErr.message));
  }

  var gsArgs = [
    '-dQUIET', '-dSAFER', '-dBATCH', '-dNOPAUSE',
    '-sDEVICE=jpeg', '-r' + OCR_RASTER_DPI, '-dJPEGQ=90',
    '-dMaxBitmap=8388608',
    '-o', path.join(pageDir, 'page_%d.jpg'),
    inputPath
  ];

  execFile(gsCommand, gsArgs, { timeout: 90000, maxBuffer: 20 * 1024 * 1024 }, function (gsErr) {
    if (gsErr) {
      removeWorkDir(pageDir);
      return callback(new Error('Could not rasterize this PDF for OCR: ' + gsErr.message));
    }

    var pageFiles;
    try {
      pageFiles = fs.readdirSync(pageDir)
      ...
      removeWorkDir(pageDir);
      return callback(new Error('Cannot read rasterized OCR pages: ' + readErr.message));
    }

    if (pageFiles.length === 0) {
      removeWorkDir(pageDir);
      return callback(new Error('No pages could be rasterized from this PDF.'));
    }

    var sharp;
    try { sharp = require('sharp'); } catch (e) { sharp = null; }

    var pages = pageFiles.map(function (fileName) {
      return { path: path.join(pageDir, fileName) };
    });

    if (!sharp) {
      return callback(null, { pageDir: pageDir, pages: pages });
    }

    var pending = pages.length;
    var failed = false;
    pages.forEach(function (page) {
      sharp(page.path).metadata().then(function (meta) {
        if (failed) return;
        page.width = meta.width;
        page.height = meta.height;
        pending--;
        if (pending === 0) callback(null, { pageDir: pageDir, pages: pages });
      }).catch(function (metaErr) {
        if (failed) return;
        failed = true;
        removeWorkDir(pageDir);
        callback(new Error('Could not read rasterized page dimensions: ' + metaErr.message));
      });
    });
  });
}

// Low-level Vision REST call using Node's built-in https module (no new npm
// dependency) - POSTs to images:annotate with the API key as a query param.
function callGoogleVisionAnnotate(requests, callback) {
  var apiKey = resolveGoogleVisionApiKey();
  if (!apiKey) {
    return callback(new Error('GOOGLE_CLOUD_VISION_API_KEY is not configured on the server.'));
  }
  ...
  var req = https.request(options, function (res) {
    ...
    res.on('end', function () {
      ...
      try { parsed = JSON.parse(body); } catch (parseErr) {
        return callback(new Error('Vision API returned an unparseable response.'));
      }
      if (res.statusCode !== 200) {
        var apiErrMsg = (parsed && parsed.error && parsed.error.message) || ('Vision API HTTP ' + res.statusCode);
        return callback(new Error('Google Cloud Vision request failed: ' + apiErrMsg));
      }
  ...
  req.on('error', function (reqErr) {
    callback(new Error('Google Cloud Vision request error: ' + reqErr.message));
  });

  req.setTimeout(60000, function () {
    req.destroy(new Error('Google Cloud Vision request timed out.'));
  });
...
// Google Cloud Vision's images:annotate endpoint rejects a request with too
// many images in one call ("Too many images per request" - documented cap
// is 16) - chunk pages into batches well under that limit so a multi-page
// scanned document never hits this ceiling regardless of its page count.
var VISION_BATCH_SIZE = 8;
...
function parseVisionAnnotateResponses(responses) {
  return responses.map(function (response) {
    if (response.error) {
      return { text: '', words: [] };
    }
    var fullText = (response.fullTextAnnotation && response.fullTextAnnotation.text) || '';
    var words = [];
    var pagesAnnotation = (response.fullTextAnnotation && response.fullTextAnnotation.pages) || [];
    pagesAnnotation.forEach(function (pageAnn) {
      (pageAnn.blocks || []).forEach(function (block) {
        (block.paragraphs || []).forEach(function (paragraph) {
          (paragraph.words || []).forEach(function (word) {
            var wordText = (word.symbols || []).map(function (s) { return s.text; }).join('');
            var vertices = (word.boundingBox && word.boundingBox.vertices) || [];
            if (!wordText || vertices.length === 0) return;
            var xs = vertices.map(function (v) { return v.x || 0; });
            var ys = vertices.map(function (v) { return v.y || 0; });
            words.push({
              text: wordText,
...
// Runs DOCUMENT_TEXT_DETECTION across all rasterized pages, chunked into
// batches of VISION_BATCH_SIZE images per Vision API call instead of one
// single request for every page (Vision rejects a request with too many
// images at once). Chunks are sent sequentially and every chunk's per-page
// results are aggregated back into one array in the original page order, so
// callers see no difference regardless of how many pages a document has.
function runVisionOcrOnPages(pages, callback) {
  var chunks = chunkArray(pages, VISION_BATCH_SIZE);
  var allPageResults = [];
  var chunkIndex = 0;

  function processNextChunk() {
    if (chunkIndex >= chunks.length) {
      return callback(null, allPageResults);
    }

    var chunk = chunks[chunkIndex];
    var requests;
    try {
      requests = chunk.map(function (page) {
        var imageBase64 = fs.readFileSync(page.path).toString('base64');
        return {
      ...
      return callback(new Error('Could not read rasterized page image: ' + readErr.message));
    }

    callGoogleVisionAnnotate(requests, function (err, result) {
      if (err) {
        return callback(new Error(
          'Vision OCR batch ' + (chunkIndex + 1) + '/' + chunks.length + ' failed: ' + err.message
        ));
      }

      var responses = (result && result.responses) || [];
      allPageResults = allPageResults.concat(parseVisionAnnotateResponses(responses));
      chunkIndex++;
      processNextChunk();
    });
  }
...
// Builds a minimal, valid .docx directly (no python-docx dependency needed)
// from real extracted OCR text - one paragraph per detected line, a page
// ---------------------------------------------------------------------------
// Spatial layout reconstruction for the Vision OCR -> DOCX path: clusters
// word-level OCR results into visual lines (Y-tolerance), detects large
// horizontal gaps to right-anchor marks/dates via a right tab stop, and
// flags centered/heading lines - instead of just dumping Vision's flat
// per-page text string as one paragraph per newline.
// ---------------------------------------------------------------------------

// Default Letter-size content width (12240 twips page - 2*1440 twips
// margins), matching this file's existing bare `<w:sectPr/>` (no explicit
// page-size override) - this IS the real right margin Word will render at.
var DOCX_CONTENT_WIDTH_TWIPS = 9360;

// Groups words into visual lines by vertical-center (Y) proximity instead of
// relying on Vision's own paragraph/block grouping - tolerance scales with
// each word's own height (floor 6px) so both body text and larger headings
// cluster correctly. Words within a line are then sorted left-to-right.
function clusterWordsIntoLines(words) {
  var sorted = words.slice().sort(function (a, b) { return a.y0 - b.y0; });
  var lines = [];
  var currentLine = null;
  var currentLineYc = 0;

  sorted.forEach(function (word) {
    var height = Math.max(1, word.y1 - word.y0);
    var yc = (word.y0 + word.y1) / 2;
    var tolerance = Math.max(6, height * 0.5);

    if (currentLine && Math.abs(yc - currentLineYc) <= tolerance) {
      currentLine.push(word);
      // Running average keeps the check stable as more words join the line.
      currentLineYc = currentLine.reduce(function (sum, w) { return sum + (w.y0 + w.y1) / 2; }, 0) / currentLine.length;
    } else {
      currentLine = [word];
      currentLineYc = yc;
...
function buildRunXml(text, bold) {
  var rPr = bold ? '<w:rPr><w:b/></w:rPr>' : '';
  return '<w:r>' + rPr + '<w:t xml:space="preserve">' + escapeXml(text) + '</w:t></w:r>';
}

// Exam-paper heading/title cues ("Section-A", "Time: Three Hours", "Total
// Marks", etc.) get bold styling even when their box height isn't notably
// larger than body text.
var HEADING_KEYWORD_PATTERN = /\b(section[\s-]?[a-z0-9]*|time\s*:|total\s*marks|question\s*paper|max(imum)?\s*marks)\b/i;

// A line that already looks like a numbered ("8.", "9)") or bulleted list
// item must never be auto-centered - only genuine short headings/titles are
// eligible for center alignment.
var LIST_ITEM_PATTERN = /^\s*(?:\d+[.)]|[\u2022\-*])\s/;

// Splits a line's words into left-to-right segments wherever the horizontal
// gap between consecutive words exceeds the given threshold - shared by the
// single-line right-tab-anchor case below and the multi-row table detector,
// so both features agree on what counts as "a column gap".
function splitLineIntoSegments(lineWords, gapThreshold) {
  var segments = [[lineWords[0]]];
  for (var i = 1; i < lineWords.length; i++) {
    var prevWord = lineWords[i - 1];
    var word = lineWords[i];
    var gap = word.x0 - prevWord.x1;
    if (gap > gapThreshold) {
      segments.push([word]);
    } else {
...
// Joins one segment's words into text and cleans stray spaces around
// brackets/punctuation that Vision's word-level tokenization can introduce.
function segmentText(segment) {
  var lineText = segment.map(function (w) { return w.text; }).join(' ').trim();
  return lineText
    .replace(/\(\s+/g, '(')
    .replace(/\s+\)/g, ')')
    .replace(/\s+([.,!?:;])/g, '$1');
...
// Builds one <w:p> for a single visual line that is NOT part of a detected
// multi-row table (see detectTableRuns/buildTableXml below). A large
// horizontal gap between consecutive words (> 35% of page width) is treated
// as a right-margin-anchored segment (marks/dates) via a right tab stop
// rather than letting it wrap onto its own line. A single, roughly
// page-centered SHORT segment that is not a list item gets center alignment
// (headings/titles only) - deliberately conservative so ordinary body text
// and numbered/bulleted list items are never mis-centered. Bold is applied
// when the line's average word height is notably above the page's average,
// or it matches a known heading keyword.
function buildLineParagraphXml(lineWords, pageWidthPx, avgWordHeight) {
  var gapThreshold = pageWidthPx > 0 ? pageWidthPx * 0.35 : Infinity;
  var segments = splitLineIntoSegments(lineWords, gapThreshold);
  var segmentTexts = segments.map(segmentText).filter(function (text) { return text.length > 0; });

  if (segmentTexts.length === 0) {
    return '';
  }

  var lineHeight = lineWords.reduce(function (sum, w) { return sum + Math.max(1, w.y1 - w.y0); }, 0) / lineWords.length;
  var fullLineText = segmentTexts.join(' ');
  var bold = (avgWordHeight > 0 && lineHeight > avgWordHeight * 1.15) || HEADING_KEYWORD_PATTERN.test(fullLineText);

  if (segmentTexts.length > 1) {
    // Right-margin anchoring: everything before the big gap stays left-
    // flowing text; the final segment (marks/date) is pushed to the right
    // margin with a right tab stop instead of dropping to a new line.
    var leftText = segmentTexts.slice(0, -1).join('   ');
    var rightText = segmentTexts[segmentTexts.length - 1];
    var tabPPr = '<w:pPr><w:tabs><w:tab w:val="right" w:pos="' + DOCX_CONTENT_WIDTH_TWIPS + '"/></w:tabs></w:pPr>';
    return '<w:p>' + tabPPr + buildRunXml(leftText, bold) + '<w:r><w:tab/></w:r>' + buildRunXml(rightText, bold) + '</w:p>';
  }

  var minX0 = Math.min.apply(null, lineWords.map(function (w) { return w.x0; }));
  var maxX1 = Math.max.apply(null, lineWords.map(function (w) { return w.x1; }));
  var lineCenter = (minX0 + maxX1) / 2;
  var pageCenter = pageWidthPx / 2;
  var isListItem = LIST_ITEM_PATTERN.test(segmentTexts[0]);
  var isShortEnoughToBeHeading = segmentTexts[0].length <= 60;
  var isCentered = pageWidthPx > 0
    && !isListItem
    && isShortEnoughToBeHeading
    && Math.abs(lineCenter - pageCenter) <= pageWidthPx * 0.03;
  var jcPPr = isCentered ? '<w:pPr><w:jc w:val="center"/></w:pPr>' : '<w:pPr><w:jc w:val="left"/></w:pPr>';

  return '<w:p>' + jcPPr + buildRunXml(segmentTexts[0], bold) + '</w:p>';
}

// Scans consecutive lines for runs of 2+ rows whose segment counts and
// column start-x positions stay consistent - i.e. a genuine multi-row grid
// (salary breakdown, offer-letter terms table), not just one isolated
// right-anchored label/value line. Returns runs in line order, each as
// { startLineIndex, endLineIndex, columnAnchors, rows }.
function detectTableRuns(lines, pageWidthPx) {
  var gapThreshold = pageWidthPx > 0 ? pageWidthPx * 0.35 : Infinity;
  var columnTolerance = pageWidthPx > 0 ? pageWidthPx * 0.08 : Infinity;
  var runs = [];
  var currentRun = null;

  function rowCandidate(lineWords) {
    var segments = splitLineIntoSegments(lineWords, gapThreshold);
    if (segments.length < 2) return null;
    var cells = segments
      .map(function (segment) { return { text: segmentText(segment), x0: segment[0].x0 }; })
      .filter(function (cell) { return cell.text.length > 0; });
  ...
  lines.forEach(function (lineWords, index) {
    var candidate = rowCandidate(lineWords);
    if (!candidate) {
      closeRun();
      return;
    }

    var matchesCurrentRun = currentRun
      && candidate.length === currentRun.columnAnchors.length
      && candidate.every(function (cell, colIndex) {
        return Math.abs(cell.x0 - currentRun.columnAnchors[colIndex]) <= columnTolerance;
      });

    if (matchesCurrentRun) {
      currentRun.rows.push(candidate);
      currentRun.endLineIndex = index;
    } else {
      closeRun();
      currentRun = {
        startLineIndex: index,
        endLineIndex: index,
        columnAnchors: candidate.map(function (cell) { return cell.x0; }),
        rows: [candidate],
      };
    }
  });
...
// Builds a genuine <w:tbl> (real rows/cells/borders/proportional column
// widths) for one detected table run - this is what makes salary breakdowns
// and offer-letter grids render as actual Word tables instead of a sequence
// of tab-separated text lines.
function buildTableXml(run, pageWidthPx) {
  var columnCount = run.columnAnchors.length;
  var anchors = run.columnAnchors.slice().sort(function (a, b) { return a - b; });
  var columnWidthsTwips = [];
  for (var c = 0; c < columnCount; c++) {
    var startPx = anchors[c];
    var endPx = c + 1 < columnCount ? anchors[c + 1] : (pageWidthPx || (startPx + 1));
    var proportion = pageWidthPx > 0 ? Math.max(0.08, (endPx - startPx) / pageWidthPx) : 1 / columnCount;
    columnWidthsTwips.push(Math.round(DOCX_CONTENT_WIDTH_TWIPS * proportion));
  }
  // Normalise so the columns sum exactly to the usable content width.
  var widthSum = columnWidthsTwips.reduce(function (a, b) { return a + b; }, 0);
  if (widthSum > 0) {
    columnWidthsTwips = columnWidthsTwips.map(function (w) { return Math.round(w * DOCX_CONTENT_WIDTH_TWIPS / widthSum); });
  }

  var borderXml = ['top', 'left', 'bottom', 'right', 'insideH', 'insideV'].map(function (side) {
    return '<w:' + side + ' w:val="single" w:sz="4" w:space="0" w:color="999999"/>';
  }).join('');
  var gridXml = columnWidthsTwips.map(function (w) { return '<w:gridCol w:w="' + w + '"/>'; }).join('');

  var rowsXml = run.rows.map(function (row) {
    var cellsXml = '';
    for (var col = 0; col < columnCount; col++) {
      var cellText = row[col] ? row[col].text : '';
      var width = columnWidthsTwips[col];
      cellsXml += '<w:tc><w:tcPr><w:tcW w:w="' + width + '" w:type="dxa"/></w:tcPr>' +
        '<w:p><w:pPr><w:jc w:val="left"/></w:pPr>' + buildRunXml(cellText, false) + '</w:p></w:tc>';
    }
    return '<w:tr>' + cellsXml + '</w:tr>';
  }).join('');

  return '<w:tbl><w:tblPr><w:tblStyle w:val="TableGrid"/><w:tblW w:w="' + DOCX_CONTENT_WIDTH_TWIPS + '" w:type="dxa"/>' +
    '<w:tblBorders>' + borderXml + '</w:tblBorders><w:tblLayout w:type="fixed"/></w:tblPr><w:tblGrid>' + gridXml + '</w:tblGrid>' +
    rowsXml + '</w:tbl><w:p><w:pPr><w:spacing w:after="120"/></w:pPr></w:p>';
}

// Detects vertical "gutters" - contiguous X-ranges with essentially no word
// bounding boxes crossing them - to split a page's words into independent
// left-to-right columns BEFORE line clustering. This is a conservative,
// single-level XY-cut (the standard document-layout-analysis technique for
// this exact problem): without it, two side-by-side blocks at the same
// vertical height (e.g. "Bill To" / "Ship To", or a 2-column resume) get
// merged into one garbled line by Y-only clustering, since nothing tells it
// those words belong to different columns rather than the same line.
//
// Deliberately conservative: a candidate gutter is only accepted if BOTH the
// column before it AND the column after it have real, substantial word
// coverage (>= 20% of the page's content height each) - this rejects a
// ragged right margin (short lines not reaching full width) being
// mistaken for a second, mostly-empty column. Falls back to a single column
// (today's exact behaviour) whenever fewer than 2 or more than 3 bands are
// found, since >3 genuine body-text columns is far more likely to be noise
// than a real layout for the documents this pipeline handles.
function detectColumnBands(words, pageWidthPx) {
  if (!pageWidthPx || words.length < 6) {
    return [{ minX: 0, maxX: pageWidthPx || 1 }];
  }

  var minY = Math.min.apply(null, words.map(function (w) { return w.y0; }));
  var maxY = Math.max.apply(null, words.map(function (w) { return w.y1; }));
  var contentHeight = Math.max(1, maxY - minY);

  var stripCount = 200;
  var stripWidth = pageWidthPx / stripCount;
  var stripOccupiedHeight = new Array(stripCount).fill(0);

  words.forEach(function (w) {
    var startStrip = Math.max(0, Math.min(stripCount - 1, Math.floor(w.x0 / stripWidth)));
    var endStrip = Math.max(0, Math.min(stripCount - 1, Math.floor(w.x1 / stripWidth)));
    var height = Math.max(1, w.y1 - w.y0);
    for (var s = startStrip; s <= endStrip; s++) {
      stripOccupiedHeight[s] += height;
    }
  });

  var gutterThreshold = contentHeight * 0.05;
  var isGutterStrip = stripOccupiedHeight.map(function (h) { return h < gutterThreshold; });
  var minGutterStrips = Math.max(1, Math.ceil((pageWidthPx * 0.04) / stripWidth));
  var minColumnOccupancy = contentHeight * 0.20;

  function sumOccupancy(fromStrip, toStrip) {
    var sum = 0;
    for (var s = fromStrip; s < toStrip; s++) sum += stripOccupiedHeight[s];
    return sum;
  }
  ...
  var i = 0;
  while (i < stripCount) {
    if (!isGutterStrip[i]) { i += 1; continue; }
    var gutterStart = i;
    while (i < stripCount && isGutterStrip[i]) i += 1;
    if (i - gutterStart < minGutterStrips) continue;

    var beforeOccupancy = sumOccupancy(bandStart, gutterStart);
    var afterOccupancy = sumOccupancy(i, stripCount);
    if (beforeOccupancy < minColumnOccupancy || afterOccupancy < minColumnOccupancy) {
      // One side is essentially empty (e.g. a ragged right margin) - not a
      // real second column, keep scanning without closing a band here.
      continue;
    }

    bands.push({ minX: bandStart * stripWidth, maxX: gutterStart * stripWidth });
    bandStart = i;
  }
  bands.push({ minX: bandStart * stripWidth, maxX: pageWidthPx });

  if (bands.length < 2 || bands.length > 3) {
    return [{ minX: 0, maxX: pageWidthPx }];
  }
  return bands;
}

// Builds paragraphs/tables for ONE column's words (already re-based so x0/x1
// are relative to that column's own left edge) - shared by the single-
// column path and each detected column in a multi-column layout, so the
// existing line-clustering/table-detection/right-tab logic never needs to
// know whether it's looking at a whole page or one column of it.
function buildColumnParagraphsXml(columnWords, columnWidthPx) {
  if (columnWords.length === 0) {
    return '';
  }
  var lines = clusterWordsIntoLines(columnWords);
  var avgWordHeight = columnWords.reduce(function (sum, w) { return sum + Math.max(1, w.y1 - w.y0); }, 0) / columnWords.length;
  var tableRuns = detectTableRuns(lines, columnWidthPx);

  var output = '';
  var runIndex = 0;
  var lineIndex = 0;
  while (lineIndex < lines.length) {
    var run = tableRuns[runIndex];
    if (run && run.startLineIndex === lineIndex) {
      output += buildTableXml(run, columnWidthPx);
      lineIndex = run.endLineIndex + 1;
      runIndex += 1;
      continue;
    }
    output += buildLineParagraphXml(lines[lineIndex], columnWidthPx, avgWordHeight);
    lineIndex += 1;
  }
  return output;
}

// Builds every paragraph/table for one page. Falls back to plain per-line
// text (no spatial reconstruction) if this page has no word-level boxes at
// all - keeps the output valid/non-empty even if Vision returned only flat
// text. Multi-column layouts are detected first (see detectColumnBands) and
// processed as independent columns in left-to-right reading order (each
// column fully top-to-bottom before the next); within each column, multi-row
// grids are still detected and rendered as real <w:tbl> structures, and
// every other line keeps the existing single-line paragraph/right-tab-
// anchor behaviour - column detection only changes HOW words are grouped
// before that existing logic runs, not what that logic itself does.
function buildPageParagraphsXml(ocrPage, pageWidthPx) {
  var words = (ocrPage && ocrPage.words) || [];
  if (words.length === 0) {
    var fallbackText = (ocrPage && ocrPage.text) || '';
    var fallbackLines = fallbackText.split(/\r?\n/).map(function (l) { return l.trim(); }).filter(function (l) { return l.length > 0; });
    if (fallbackLines.length === 0) {
      return '<w:p><w:r><w:t xml:space="preserve">(No text detected on this page.)</w:t></w:r></w:p>';
    }
    return fallbackLines.map(function (line) { return '<w:p>' + buildRunXml(line, false) + '</w:p>'; }).join('');
  }

  var columnBands = detectColumnBands(words, pageWidthPx);
  if (columnBands.length <= 1) {
    return buildColumnParagraphsXml(words, pageWidthPx);
  }

  var output = '';
  columnBands.forEach(function (band, bandIndex) {
    var columnWords = words
      .filter(function (w) {
        var centerX = (w.x0 + w.x1) / 2;
        return centerX >= band.minX && centerX < band.maxX;
      })
      .map(function (w) { return { x0: w.x0 - band.minX, x1: w.x1 - band.minX, y0: w.y0, y1: w.y1, text: w.text }; });

    var columnXml = buildColumnParagraphsXml(columnWords, band.maxX - band.minX);
    output += columnXml;
    if (columnXml && bandIndex < columnBands.length - 1) {
      // Word doesn't lay out true side-by-side newspaper columns without a
      // dedicated section/column definition (out of scope here) - each
      // column is instead emitted fully, in reading order, with a small
      // spacer between columns so the content stays correctly separated
      // and readable rather than interleaved/garbled.
      output += '<w:p><w:pPr><w:spacing w:after="60"/></w:pPr></w:p>';
    }
  });
...
// Builds a spatially-reconstructed .docx (hand-built OOXML + archiver, same
// pattern as the rest of this file) from Vision OCR results - clusters
// word-level bounding boxes into lines, right-anchors marks/dates, and
// centers/bolds headings - instead of dumping Vision's flat per-page text as
// one paragraph per newline.
function buildSimpleDocxFromPages(pages, ocrPages, outputPath, callback) {
  var archiver;
  try { archiver = require('archiver'); } catch (e) {
    return callback(new Error('archiver module not available. Re-run npm install'));
  }

  var bodyParts = ocrPages.map(function (ocrPage, index) {
    var pageWidthPx = (pages[index] && pages[index].width) || 0;
    var paragraphsXml = buildPageParagraphsXml(ocrPage, pageWidthPx);
    var isLast = index === ocrPages.length - 1;
    var pageBreak = isLast ? '' : '<w:p><w:r><w:br w:type="page"/></w:r></w:p>';
    return paragraphsXml + pageBreak;
  }).join('');

  var documentXml = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
    '<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">' +
    '<w:body>' + bodyParts + '<w:sectPr/></w:body></w:document>';
  ...
  output.on('close', function () { settle(null); });
  archive.on('error', function (archErr) { settle(archErr); });
...
// Builds a "sandwich" searchable PDF: each rasterized page image is embedded
// as the visible background, with Vision's word-level OCR results drawn as
// fully transparent (opacity: 0) text at the matching position - the text
// is real content-stream text (selectable/searchable/copyable) even though
// it's visually invisible, the standard technique used by OCR-to-searchable-
// PDF tools. Word-level bounding boxes come straight from Vision, so no
// custom text-layout logic is needed - only a pixel -> PDF point unit
// conversion (dividing by OCR_RASTER_DPI) and a Y-axis flip (PDF origin is
// bottom-left; image origin is top-left). A single malformed word is
// skipped rather than failing the whole page.
function buildSearchablePdfFromOcr(pages, ocrPages, outputPath, callback) {
  var PDFDocument, StandardFonts, rgb;
  try {
    var pdfLib = require('pdf-lib');
    ...
    return pdfDoc.embedFont(StandardFonts.Helvetica).then(function (font) {
      ...
      pages.forEach(function (page, index) {
        chain = chain.then(function () {
          var imageBytes = fs.readFileSync(page.path);
          return pdfDoc.embedJpg(imageBytes).then(function (embeddedImage) {
            var widthPx = page.width || embeddedImage.width;
            var heightPx = page.height || embeddedImage.height;
            var pageWidthPoints = (widthPx / OCR_RASTER_DPI) * 72;
            var pageHeightPoints = (heightPx / OCR_RASTER_DPI) * 72;
            var pdfPage = pdfDoc.addPage([pageWidthPoints, pageHeightPoints]);

            pdfPage.drawImage(embeddedImage, { x: 0, y: 0, width: pageWidthPoints, height: pageHeightPoints });

            var ocrPage = ocrPages[index] || { words: [] };
            (ocrPage.words || []).forEach(function (word) {
              try {
                var xPoints = (word.x0 / OCR_RASTER_DPI) * 72;
                var boxHeightPoints = ((word.y1 - word.y0) / OCR_RASTER_DPI) * 72;
                var yTopPoints = (word.y0 / OCR_RASTER_DPI) * 72;
                var yPoints = pageHeightPoints - yTopPoints - boxHeightPoints;
                var fontSize = Math.max(4, boxHeightPoints * 0.85);
                ...
                // Skip a single malformed word rather than failing the whole page.
              }
            });
          });
        });
      });
      return chain.then(function () { return pdfDoc; });
    });
  }).then(function (pdfDoc) {
    return pdfDoc.save();
  }).then(function (pdfBytes) {
    fs.writeFile(outputPath, Buffer.from(pdfBytes), function (writeErr) {
      if (writeErr) return callback(writeErr);
      callback(null);
    });
  }).catch(function (err) {
...
// Orchestrates: rasterize -> Vision OCR -> "sandwich" searchable PDF.
function ocrScannedPdfToSearchablePdf(inputPath, outputPath, callback) {
  rasterizePdfPagesForOcr(inputPath, function (rasterErr, rasterResult) {
    if (rasterErr) return callback(rasterErr);

    runVisionOcrOnPages(rasterResult.pages, function (ocrErr, ocrPages) {
      if (ocrErr) {
        removeWorkDir(rasterResult.pageDir);
        return callback(ocrErr);
      }

      buildSearchablePdfFromOcr(rasterResult.pages, ocrPages, outputPath, function (buildErr) {
        removeWorkDir(rasterResult.pageDir);
        if (buildErr) return callback(buildErr);
        callback(null, { pageCount: rasterResult.pages.length });
      });
    });
  });
}

// Orchestrates: rasterize -> Vision OCR -> spatially-reconstructed DOCX.
function ocrScannedPdfToDocx(inputPath, outputPath, callback) {
  rasterizePdfPagesForOcr(inputPath, function (rasterErr, rasterResult) {
    if (rasterErr) return callback(rasterErr);

    runVisionOcrOnPages(rasterResult.pages, function (ocrErr, ocrPages) {
      removeWorkDir(rasterResult.pageDir);
      if (ocrErr) return callback(ocrErr);

      buildSimpleDocxFromPages(rasterResult.pages, ocrPages, outputPath, function (buildErr) {
        if (buildErr) return callback(buildErr);
        callback(null, { pageCount: rasterResult.pages.length });
      });
    });
...
// Quick pre-flight check: does this PDF have a real, meaningful text layer?
// Used to route 100% scanned/photostat PDFs (zero digital text) to a clean,
// actionable error instead of feeding them into pdf2docx/LibreOffice, both
// of which are guaranteed to fail on a pure scan and only waste wall clock
// and memory. Always "fails open" (hasText: true) on any error so a broken
// check never blocks a normal, text-based conversion.
function checkPdfHasSelectableText(inputPath, callback) {
  var execFile = require('child_process').execFile;
  var pythonBin = resolvePython3Binary();
  var scriptPath = path.join(__dirname, 'check_pdf_text.py');

  execFile(pythonBin, [scriptPath, inputPath], { timeout: 15000, maxBuffer: 5 * 1024 * 1024 }, function (err, stdout, stderr) {
    if (err) {
      console.error('[convert-pdf-to-docx][TextCheck] Check failed, proceeding as normal (non-scanned) conversion: ' + err.message + (stderr ? (' | stderr: ' + stderr) : ''));
      return callback({ hasText: true, checkFailed: true });
    }

    var parsed = null;
    try { parsed = JSON.parse(stdout); } catch (parseErr) { parsed = null; }

    if (!parsed || parsed.error) {
      console.error('[convert-pdf-to-docx][TextCheck] Check reported an error, proceeding as normal (non-scanned) conversion: ' + (parsed && parsed.error ? parsed.error : 'unparseable output'));
      return callback({ hasText: true, checkFailed: true });
    }
...
// Tries pdf2docx first (highest fidelity); gracefully falls back to headless
// LibreOffice for normal, text-based PDFs. 100% scanned/photostat PDFs (zero
// selectable text - see checkPdfHasSelectableText() above) are never handed
// to either engine, since both are guaranteed to fail on them - instead the
// callback receives an Error flagged with `.isScanned = true` so the route
// can return a clean, actionable response pointing the user at the OCR/
// Extract Images tool instead of attempting a fragile image-to-docx hack.
// Tier 3: LibreOffice-independent fallback via the shared Google Cloud
// Vision OCR engine - rasterizes with Ghostscript, extracts text via Vision,
// packages a pure-Node/archiver OOXML .docx (see ocrScannedPdfToDocx). Called
// by convertPdfToDocxHighFidelity() whenever pdf2docx AND LibreOffice both
// fail for ANY reason. Always returns a callback error that CLEARLY states
// WHY Tier 3 itself could not run (missing API key / unknown page count /
// global cap reached / Vision failure) instead of silently re-surfacing the
// earlier LibreOffice error - a missing GOOGLE_CLOUD_VISION_API_KEY must be
// immediately diagnosable from the API response itself, not just look like
// "LibreOffice is broken" forever.
function convertPdfToDocxViaVisionOcr(inputPath, pageCount, priorErr, callback) {
  if (!resolveGoogleVisionApiKey()) {
    console.error('[convert-pdf-to-docx][Tier3:VisionOCR] Skipped: GOOGLE_CLOUD_VISION_API_KEY not configured - all conversion tiers exhausted.');
    return callback(new Error(
      'PDF to Word conversion failed: pdf2docx and LibreOffice both failed (' +
      (priorErr ? priorErr.message : 'unknown error') +
      '), and the Google Cloud Vision OCR fallback is NOT configured on this server (missing GOOGLE_CLOUD_VISION_API_KEY environment variable). Check GET /api/info -> visionOcrConfigured.'
    ));
  }
  if (!pageCount) {
    console.error('[convert-pdf-to-docx][Tier3:VisionOCR] Skipped: could not determine page count - all conversion tiers exhausted.');
    return callback(new Error("PDF to Word conversion failed: could not determine this PDF's page count for the Vision OCR fallback."));
  }

  var reservation = reserveOcrGlobalPages(pageCount);
  if (!reservation.allowed) {
    console.error('[convert-pdf-to-docx][Tier3:VisionOCR] Skipped: global monthly OCR page cap reached - all conversion tiers exhausted.');
    return callback(new Error('PDF to Word conversion failed: the shared monthly AI OCR page limit has been reached. Resets on the 1st of next month.'));
  }

  var jobId = 'p2d_vision_' + Date.now() + '_' + Math.round(Math.random() * 1e6);
  var outDir = path.join(UPLOAD_DIR, jobId);
  try {
    fs.mkdirSync(outDir, { recursive: true });
  } catch (dirErr) {
    console.error('[convert-pdf-to-docx][Tier3:VisionOCR] Cannot create work directory: ' + dirErr.message);
    return callback(new Error('PDF to Word conversion failed: could not prepare the Vision OCR work directory (' + dirErr.message + ').'));
  }
  var visionOutputPath = path.join(outDir, 'document.docx');

  ocrScannedPdfToDocx(inputPath, visionOutputPath, function (ocrErr) {
    if (ocrErr) {
      removeWorkDir(outDir);
      console.error('[convert-pdf-to-docx][Tier3:VisionOCR] Failed: ' + ocrErr.message + ' - all conversion tiers exhausted.');
      return callback(new Error('PDF to Word conversion failed: the Google Cloud Vision OCR fallback also failed (' + ocrErr.message + ').'));
    }

    var ocrDocxBytes;
    try {
      ocrDocxBytes = fs.readFileSync(visionOutputPath);
    } catch (readErr) {
      removeWorkDir(outDir);
      console.error('[convert-pdf-to-docx][Tier3:VisionOCR] Cannot read output: ' + readErr.message);
      return callback(new Error('PDF to Word conversion failed: could not read the Vision OCR output (' + readErr.message + ').'));
    }

    removeWorkDir(outDir);
    callback(null, ocrDocxBytes);
  });
}

function convertPdfToDocxHighFidelity(inputPath, callback) {
  checkPdfHasSelectableText(inputPath, function (textInfo) {
    var isScanned = textInfo && textInfo.hasText === false && !textInfo.checkFailed;

    if (isScanned) {
      console.error('[convert-pdf-to-docx] Detected 100% scanned/photostat PDF (textLength=' + textInfo.textLength + ' across ' + textInfo.sampledPages + '/' + textInfo.pageCount + ' sampled pages) - no digital text to convert.');
      var scannedError = new Error('This document is a scanned image PDF with no selectable text. Use OCR or Extract Images tool.');
      scannedError.isScanned = true;
      return callback(scannedError);
    }

    var pageCount = (textInfo && textInfo.pageCount) || 0;

    function proceedWithPageCount(resolvedPageCount) {
      convertPdfToDocxWithPdf2docx(inputPath, function (pdf2docxErr, docxBytes) {
        if (!pdf2docxErr && docxBytes && docxBytes.length > 0) {
          return callback(null, docxBytes, 'pdf2docx');
        }

        console.error('[convert-pdf-to-docx][Tier1:pdf2docx] Failed: ' + (pdf2docxErr ? pdf2docxErr.message : 'unknown error') + ' - falling back to Tier 2 (LibreOffice).');

        convertPdfToDocxWithLibreOffice(inputPath, function (loErr, loDocxBytes) {
          if (!loErr && loDocxBytes && loDocxBytes.length > 0) {
            return callback(null, loDocxBytes, 'libreoffice');
          }

          console.error('[convert-pdf-to-docx][Tier2:LibreOffice] Failed: ' + (loErr ? loErr.message : 'unknown error') + ' - falling back to Tier 3 (Google Cloud Vision OCR).');

          convertPdfToDocxViaVisionOcr(inputPath, resolvedPageCount, loErr, function (visionErr, visionDocxBytes) {
            if (visionErr) {
              return callback(visionErr);
            }
            callback(null, visionDocxBytes, 'vision-ocr');
          });
        });
      });
    }
    ...
    if (pageCount) {
      return proceedWithPageCount(pageCount);
    }

    // The Python/PyMuPDF check didn't yield a page count (checkFailed, e.g.
    // python3 unresolvable at runtime) - fall back to pdf-lib (pure Node,
    // already used by handleSharedOcrRoute for the same reason) BEFORE
    // Tier 3 needs it, so a broken Python environment never blocks the
    // Vision OCR fallback with a false "could not determine page count".
    getPdfPageCountViaPdfLib(inputPath, function (countErr, fallbackCount) {
      if (countErr) {
        console.error('[convert-pdf-to-docx] pdf-lib page-count fallback also failed: ' + countErr.message);
      }
      proceedWithPageCount(fallbackCount || 0);
    });
  });
}

// ---------------------------------------------------------------------------
// Optical pre-deskew + auto-enhance: straightens rasterized scan pages and
// stretches contrast via ImageMagick before they're archived, so any
// downstream OCR/text-extraction on these images starts from a cleaner
// source. Best-effort only - if ImageMagick is unavailable or a single page
// fails, that page is left as-is and the batch still completes.
// ---------------------------------------------------------------------------
function deskewAndEnhanceImage(imagePath, callback) {
  var execFile = require('child_process').execFile;
  var magickBin = resolveImageMagickBinary();
  ...
  execFile(magickBin, [
    ...
    '-auto-level',
    imagePath
  ], { timeout: 20000, maxBuffer: 20 * 1024 * 1024 }, function (err) {
    if (err) {
      console.warn('[deskew] Skipped enhance for ' + path.basename(imagePath) + ': ' + err.message);
    }
    callback();
  });
}

function deskewAndEnhancePages(pageFiles, tempPageDir, callback) {
  var index = 0;
  function next() {
    if (index >= pageFiles.length) return callback();
    var filePath = path.join(tempPageDir, pageFiles[index]);
    index++;
    deskewAndEnhanceImage(filePath, next);
  }
  next();
}

// ---------------------------------------------------------------------------
// Excel (XLSX/XLS) -> CSV or PDF via headless LibreOffice. Reuses the same
// isolated-profile-dir + concurrency-semaphore pattern as
// convertPdfToDocxWithLibreOffice so multi-thousand-row billing sheets are
// converted natively (outside Node's V8 heap) instead of being parsed in
// browser memory.
// ---------------------------------------------------------------------------
function convertSpreadsheetWithLibreOffice(inputPath, targetFormat, callback) {
  if (!isLibreOfficeAvailable()) {
    return callback(new Error('LibreOffice (soffice) is not installed or could not be found on this server - spreadsheet conversion is unavailable.'));
  }

  var execFile = require('child_process').execFile;
  var sofficeBin = resolveLibreOfficeBinary();
  var jobId = 'xlsx_' + Date.now() + '_' + Math.round(Math.random() * 1e6);
  var profileDir = path.join(UPLOAD_DIR, 'profile_' + jobId);
  var outDir = path.join(UPLOAD_DIR, 'out_' + jobId);
  ...
  acquireLibreOfficeSlot(function () {
    var convertFilter = targetFormat === 'pdf'
      ? 'pdf:calc_pdf_Export'
      : targetFormat === 'xlsx'
    ...
    execFile(sofficeBin, args, { timeout: 100000, maxBuffer: 20 * 1024 * 1024 }, function (err) {
      ...
      if (err) {
        removeWorkDir(profileDir);
        removeWorkDir(outDir);
        return callback(new Error('LibreOffice spreadsheet conversion failed: ' + err.message));
      }

      var baseName = path.parse(inputPath).name;
      var outputPath = path.join(outDir, baseName + '.' + targetFormat);

      if (!fs.existsSync(outputPath) || fileSize(outputPath) === 0) {
        removeWorkDir(profileDir);
        removeWorkDir(outDir);
        return callback(new Error('LibreOffice did not produce a ' + targetFormat.toUpperCase() + ' output for this spreadsheet.'));
      }
      ...
      removeWorkDir(profileDir);
      removeWorkDir(outDir);
      callback(null, outputBytes);
    });
  });
}

// ---------------------------------------------------------------------------
// Multi-sheet-aware XLSX -> CSV export via Python/openpyxl (streaming/
// read-only mode - no row-count ceiling, handles 50,000+ row billing
// sheets). Every sheet becomes its own "<SheetName>.csv" file; the caller
// decides whether to return one file directly or zip several together.
// Legacy .xls is NOT handled here (openpyxl only reads OOXML .xlsx) - it
// still goes through the single-sheet convertSpreadsheetWithLibreOffice path.
// ---------------------------------------------------------------------------
function resolvePython3Binary() {
  if (process.env.PYTHON3_BIN) {
    return process.env.PYTHON3_BIN;
  }

  // Absolute paths first - same defensive pattern as resolveLibreOfficeBinary(),
  // protects against a bare `python3` PATH lookup failing at runtime (ENOENT)
  // even when it would resolve fine in a full Docker-build-time shell.
  var candidates = process.platform === 'win32'
    ? ['python']
...
function convertXlsxToCsvPerSheet(inputPath, callback) {
  var execFile = require('child_process').execFile;
  var jobId = 'xlsx2csv_' + Date.now() + '_' + Math.round(Math.random() * 1e6);
  var outDir = path.join(UPLOAD_DIR, 'xlsx2csv_' + jobId);

  try {
    fs.mkdirSync(outDir, { recursive: true });
  } catch (dirErr) {
    return callback(new Error('Cannot create work directory: ' + dirErr.message));
  }

  var pythonBin = resolvePython3Binary();
  var scriptPath = path.join(__dirname, 'xlsx_to_csv.py');

  execFile(pythonBin, [scriptPath, inputPath, outDir], { timeout: 100000, maxBuffer: 20 * 1024 * 1024 }, function (err, stdout, stderr) {
    if (err) {
      removeWorkDir(outDir);
      return callback(new Error('Multi-sheet CSV export failed: ' + (stderr || err.message)));
    }
    ...
    try {
      ...
      removeWorkDir(outDir);
      return callback(new Error('Multi-sheet CSV export produced unreadable output.'));
    }
    ...
    var sheetFiles = parsed.sheets || [];
    if (sheetFiles.length === 0) {
      removeWorkDir(outDir);
      return callback(new Error('No sheets were found in this workbook.'));
    }

    callback(null, { outDir: outDir, sheetFiles: sheetFiles });
  });
}

// Bundles a list of existing files into an in-memory ZIP buffer using the
// same archiver settings as convertPdfToImagesArchive.
function zipFilesToBuffer(filePaths, callback) {
  var archiver;
  try { archiver = require('archiver'); } catch (e) {
    return callback(new Error('archiver module not available. Re-run npm install'));
  }

  var zipPath = path.join(UPLOAD_DIR, 'zip_' + Date.now() + '_' + Math.round(Math.random() * 1e6) + '.zip');
  var output = fs.createWriteStream(zipPath);
  var archive = archiver('zip', { zlib: { level: 9 } });
  var settled = false;

  function settle(err, zipBytes) {
    if (settled) return;
    settled = true;
    cleanupFile(zipPath);
    callback(err, zipBytes);
  }
  ...
  output.on('close', function () {
    ...
    try {
      ...
      return settle(new Error('Cannot read generated archive: ' + readErr.message));
    }
    settle(null, zipBytes);
  });

  archive.on('error', function (archErr) {
    settle(new Error('Archive creation failed: ' + archErr.message));
  });
...
// ---------------------------------------------------------------------------
// Generic PDF -> Image (JPG/PNG) conversion: rasterizes every page with
// Ghostscript, then streams a ZIP archive back. Server-side so large/complex
// PDFs never need to be rendered in browser memory.
// ---------------------------------------------------------------------------
function convertPdfToImagesArchive(inputPath, targetFormat, callback) {
  var execFile = require('child_process').execFile;
  var archiver;
  try { archiver = require('archiver'); } catch (e) {
    return callback(new Error('archiver module not available. Re-run npm install'));
  ...
  var gsCommand = resolveGhostscriptBinary();
  var jobId = 'cvt_' + Date.now() + '_' + Math.round(Math.random() * 1e6);
  var tempPageDir = path.join(UPLOAD_DIR, 'pages_' + jobId);

  try {
    fs.mkdirSync(tempPageDir, { recursive: true });
  } catch (dirErr) {
    return callback(new Error('Cannot create temp directory: ' + dirErr.message));
  }

  var device = targetFormat === 'png' ? 'png16m' : 'jpeg';
  var gsArgs = [
    '-dQUIET',
    '-dSAFER',
    '-dBATCH',
  ...
  if (device === 'jpeg') {
    gsArgs.push('-dJPEGQ=88');
  }
  gsArgs.push('-o', path.join(tempPageDir, 'page_%d.' + targetFormat), inputPath);

  execFile(gsCommand, gsArgs, { timeout: 100000, maxBuffer: 20 * 1024 * 1024 }, function (gsErr) {
    if (gsErr) {
      removeWorkDir(tempPageDir);
      return callback(new Error('PDF to image conversion failed: ' + gsErr.message));
    }

    var pageFiles;
    try {
      pageFiles = fs.readdirSync(tempPageDir)
        .filter(function (f) { return f.endsWith('.' + targetFormat); })
        .sort(function (a, b) {
          var numA = parseInt(a.match(/\d+/)[0], 10);
          var numB = parseInt(b.match(/\d+/)[0], 10);
          return numA - numB;
      ...
      removeWorkDir(tempPageDir);
      return callback(new Error('Cannot read converted pages: ' + readDirErr.message));
    }

    if (pageFiles.length === 0) {
      removeWorkDir(tempPageDir);
      return callback(new Error('No pages were generated from this PDF.'));
    }

    // Optical pre-enhance pass: deskew + auto-contrast each rasterized page
    // before archiving (see deskewAndEnhancePages above).
    deskewAndEnhancePages(pageFiles, tempPageDir, function () {
      var zipPath = path.join(UPLOAD_DIR, jobId + '.zip');
      var output = fs.createWriteStream(zipPath);
      var archive = archiver('zip', { zlib: { level: 9 } });
      var settled = false;
      ...
      archive.pipe(output);
      pageFiles.forEach(function (fileName, index) {
        archive.file(path.join(tempPageDir, fileName), {
          name: 'page_' + String(index + 1).padStart(3, '0') + '.' + targetFormat
        });
      });
      archive.finalize();
    });
...
app.post('/api/admin/plan-catalog', requireAdmin, function (req, res) {
  ...
  var next = {
    ...
    user_quotas_by_plan: sanitizePlanCatalogQuotas(payload.user_quotas_by_plan, planCatalogConfig.user_quotas_by_plan),
    voice_quotas_by_plan: sanitizePlanCatalogQuotas(payload.voice_quotas_by_plan, planCatalogConfig.voice_quotas_by_plan),
    ocr_quotas_by_plan: sanitizePlanCatalogQuotas(payload.ocr_quotas_by_plan, planCatalogConfig.ocr_quotas_by_plan)
  };
...
// Official GSTR-1 multi-tab Excel export (GST Offline Tool schema: b2cs/b2b/hsn/doc_issue).
app.get('/api/admin/gst-report/export-excel', requireAdmin, function (req, res) {
  var request = readSalesReportRequest(req);
  var report = buildSalesReportData(request.range, request.fromDate, request.toDate, request.filters);
  var workbook = buildGstr1ExcelWorkbook(report);

  res.set('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  res.set('Content-Disposition', 'attachment; filename="GSTR1_MultiTab_' + encodeURIComponent(request.range) + '.xlsx"');

  workbook.xlsx.write(res).then(function () {
    res.end();
  }).catch(function (err) {
    console.error('GSTR-1 Excel export failed:', err && err.message ? err.message : err);
    if (!res.headersSent) {
      res.status(500).json({ success: false, error: 'Failed to generate the GSTR-1 Excel export.' });
    }
  });
});
...
// Lightweight health/uptime probe for external keep-alive pings and Render's
// own health checks - intentionally does no heavy work (no disk/DB/API
// calls) so it stays fast even if other parts of the server are under load.
// Registered at both paths since different monitors default to either one.
...
app.get('/api/info', function (req, res) {
  res.json({
    ...
    libreOfficePath: isLibreOfficeAvailable() ? resolveLibreOfficeBinary() : null,
    pdf2docxAvailable: isPdf2docxAvailable(),
    pdf2docxPath: isPdf2docxAvailable() ? resolvePdf2docxBinary() : null,
    visionOcrConfigured: Boolean(resolveGoogleVisionApiKey())
  });
});

// ==========================================
// AI RESUME BUILDER - REAL AI ASSIST ROUTE
// ==========================================
// Used by AI Resume Builder's "AI Assist" (field polish), "Refine with AI"
// and "Company Insights" features. The Flutter client only calls this
// after confirming the signed-in user is on a paid plan
// (PlanCatalogConfig.isPaidPlan) - free-tier users never reach this route,
// so it never costs anything on the free tier.
// In-memory rate limiter for /api/ai-resume-assist, keyed by email: max
// AI_RESUME_ASSIST_RATE_LIMIT_MAX requests per rolling hour. This resets on
// server restart and is per-instance only - a cheap mitigation against a
// paid session being reused/abused, not the primary access control (the
// paid-plan check in the route below is that).
var aiResumeAssistRateLimits = new Map();
var AI_RESUME_ASSIST_RATE_LIMIT_MAX = 20;
var AI_RESUME_ASSIST_RATE_LIMIT_WINDOW_MS = 60 * 60 * 1000;

function isAiResumeAssistRateLimited(email) {
  var now = Date.now();
  var timestamps = (aiResumeAssistRateLimits.get(email) || []).filter(function (ts) {
    return now - ts < AI_RESUME_ASSIST_RATE_LIMIT_WINDOW_MS;
  });
  if (timestamps.length >= AI_RESUME_ASSIST_RATE_LIMIT_MAX) {
    aiResumeAssistRateLimits.set(email, timestamps);
    return true;
  }
  timestamps.push(now);
  aiResumeAssistRateLimits.set(email, timestamps);
...
var ANTHROPIC_RESUME_ASSIST_PROMPTS = {
  polish_field: function (data) {
    return "You are a professional resume writer. Rewrite the following resume section labelled '" + (data.fieldLabel || '') + "' so it reads as polished, concise, achievement-focused resume language for the target role '" + (data.role || 'the target role') + "'. Stay strictly truthful to the input - never invent employers, numbers, dates, or achievements that are not implied by it. Return ONLY the rewritten text, no preamble.\n\nCurrent content:\n" + (data.currentValue || '');
  },
  refine_text: function (data) {
    return "Improve the clarity, tone, and professional impact of the following text for a job application, without changing any facts, names, or figures it contains. Return ONLY the improved text, no preamble.\n\nText:\n" + (data.currentValue || '');
  },
  company_insights: function (data) {
    return "Give a realistic overview of the company '" + (data.companyName || '') + "' for a job seeker: typical work culture, an approximate salary range for mid-level roles in India if you have reasonable knowledge of it, common pros, and common cons mentioned in public reviews. If you are not confident about specifics for this exact company, say so plainly instead of guessing - never state a specific numeric rating or salary figure you are not reasonably confident about. Keep it under 180 words.";
  }
};

app.post('/api/ai-resume-assist', function (req, res) {
  var apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) {
    return res.status(500).json({ success: false, error: 'ANTHROPIC_API_KEY is not configured on server.' });
  }
  ...
  var hasPlan = Boolean(user) && (String(user.planId || '').trim() !== '' || String(user.planName || '').trim() !== '');
  var isFreePlan = Boolean(user) && String(user.planName || '').trim().toLowerCase() === 'free';
  var planStatus = user ? getUserPlanStatus(user) : 'inactive';
  if (!user || !hasPlan || isFreePlan || planStatus !== 'active') {
    return res.status(403).json({ success: false, error: 'This feature requires an active paid plan.' });
  }

  if (isAiResumeAssistRateLimited(email)) {
    return res.status(429).json({ success: false, error: 'Too many AI requests. Please try again later.' });
  }

  var action = (req.body && req.body.action) || '';
  var data = (req.body && req.body.data) || {};
  var promptBuilder = ANTHROPIC_RESUME_ASSIST_PROMPTS[action];
  if (!promptBuilder) {
    return res.status(400).json({ success: false, error: 'Unknown or unsupported action.' });
  }

  var prompt = promptBuilder(data);
  ...
  fetch('https://api.anthropic.com/v1/messages', {
    ...
    .then(function (result) {
      if (!result.ok) {
        var errMsg = (result.json && result.json.error && result.json.error.message) || 'AI request failed.';
        console.warn('[AI-RESUME-ASSIST] Anthropic API error: ' + errMsg);
        return res.status(502).json({ success: false, error: 'AI request failed. Please try again.' });
      }
      var text = '';
      var content = result.json && result.json.content;
    ...
    .catch(function (err) {
      console.warn('[AI-RESUME-ASSIST] Request failed: ' + err.message);
      return res.status(502).json({ success: false, error: 'AI request failed. Please try again.' });
    });
});
...
app.post('/api/create-order', function (req, res) {
  ...
  if (!razorpay) {
    if (process.env.ALLOW_LOCAL_CHECKOUT_FALLBACK !== 'true') {
      return res.status(503).json({ success: false, error: 'Payment gateway is not configured. Please contact support.' });
    }
    console.warn('[SECURITY] ' + new Date().toISOString() + ' ALLOW_LOCAL_CHECKOUT_FALLBACK is active - granting access for orderId=' + localOrderId + ' WITHOUT a verified Razorpay payment. This must never fire in production.');
    upsertUserAccount({
      email: normalizedBilling.email || '',
      planId: planId,
      planName: planName,
...
app.post('/api/verify-payment', function (req, res) {
  ...
  if (razorpay) {
    ...
    console.warn('[SECURITY] ' + new Date().toISOString() + ' ALLOW_LOCAL_CHECKOUT_FALLBACK is active - verifying orderId=' + orderId + ' WITHOUT a real Razorpay signature check. This must never fire in production.');
  }
...
// ---------------------------------------------------------------------------
// Voice Command Top-Up Packs - isolated Razorpay routes (deliberately separate
// from /api/create-order and /api/verify-payment to avoid any risk to the
// existing subscription payment flow). Reuses the same tax/invoice/order
// helpers so invoices remain consistent with the rest of the platform.
// ---------------------------------------------------------------------------
app.post('/api/voice-topup/create-order', function (req, res) {
  var packId = String(req.body.packId || '').trim();
  var packName = String(req.body.packName || 'Voice Top-Up').trim();
  var credits = parseInt(req.body.credits, 10);
  var amount = parseInt(req.body.amount, 10);
  var currency = (req.body.currency || 'INR').toUpperCase();
  var billing = req.body.billing || {};

  if (!packId || !amount || amount <= 0 || !credits || credits <= 0) {
    return res.status(400).json({ success: false, error: 'Invalid voice top-up pack request.' });
  }
  ...
  var localOrderId = 'local-topup-' + Date.now() + '-' + Math.random().toString(36).slice(2, 6);
  var taxBreakdown = resolveTaxBreakdown(amount, normalizedBilling, currency);
  var receipt = 'voice-topup-' + packId + '-' + Date.now();

  var pendingOrderRecord = {
    itemType: 'voice-topup',
    localOrderId: localOrderId,
    razorpayOrderId: null,
    orderId: localOrderId,
    packId: packId,
  ...
  logAuditEvent(normalizedBilling.email || 'guest', 'voice-topup-checkout-created', {
    orderId: localOrderId,
    packId: packId,
    credits: credits,
    amount: amount,
  ...
  if (!razorpay) {
    return res.json({ success: true, order_id: localOrderId, amount: amount, currency: currency, localOnly: true, taxBreakdown: taxBreakdown });
  }
  ...
  razorpay.orders.create({
    ...
    if (err) {
      console.error('Razorpay voice-topup create-order error:', err);
      var status = 500;
      if (err.statusCode === 401) status = 401;
      return res.status(status).json({ success: false, error: err.error || 'Unable to create Razorpay order.' });
    }
    ...
    pendingOrderRecord.orderId = order.id;
    registerPendingOrder(order.id, pendingOrderRecord);
    res.json({ success: true, order_id: order.id, amount: order.amount, currency: order.currency, taxBreakdown: taxBreakdown });
  });
});

app.post('/api/voice-topup/verify', function (req, res) {
  var orderId = req.body.order_id;
  var paymentId = req.body.payment_id;
  var signature = req.body.signature;
  ...
  var pendingOrder = resolvePendingOrder(orderId);
  if (!pendingOrder || pendingOrder.itemType !== 'voice-topup') {
    return res.status(404).json({ success: false, error: 'Voice top-up order not found.' });
  }

  var transactionId = 'txn-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8);
  var recipientEmail = pendingOrder.billing && pendingOrder.billing.email ? pendingOrder.billing.email : '';
  var transaction = {
    transactionId: transactionId,
    invoiceNumber: generateNextInvoiceNumber(),
    itemType: 'voice-topup',
    packId: pendingOrder.packId,
    packName: pendingOrder.packName,
    credits: pendingOrder.credits,
    orderId: orderId,
    paymentId: paymentId || 'local-' + Date.now(),
    planId: pendingOrder.packId,
    planName: pendingOrder.packName + ' - Voice Command Top-Up',
    amount: pendingOrder.amount / 100,
    currency: pendingOrder.currency,
    totalAmount: pendingOrder.amount / 100,
    taxBreakdown: pendingOrder.taxBreakdown,
  ...
  creditVoiceCommands(recipientEmail, pendingOrder.credits);

  logAuditEvent(recipientEmail || 'guest', 'voice-topup-verified', { transactionId: transaction.transactionId, packId: pendingOrder.packId, credits: pendingOrder.credits });

  res.json({
    success: true,
    message: 'Voice command top-up verified successfully.',
    transactionId: transaction.transactionId,
    creditsAdded: pendingOrder.credits,
    invoiceUrl: '/api/user/invoice/' + transaction.transactionId
  });
});
...
// Model resilience chain: try the requested Pay-As-You-Go primary/fallback
// models first (distinct quota pools), then fall back to the alias already
// confirmed to work against this project's API key, so a model-name/quota
// hiccup on either named tier never fully breaks voice commands.
var GEMINI_MODEL_PRIMARY = (process.env.GEMINI_MODEL_PRIMARY || process.env.GEMINI_MODEL || 'gemini-1.5-flash').trim();
var GEMINI_MODEL_FALLBACK = (process.env.GEMINI_MODEL_FALLBACK || 'gemini-1.5-flash-8b').trim();
var GEMINI_MODEL_SAFETY_NET = (process.env.GEMINI_MODEL_SAFETY_NET || 'gemini-flash-latest').trim();
var GEMINI_MODEL_CHAIN = [GEMINI_MODEL_PRIMARY, GEMINI_MODEL_FALLBACK, GEMINI_MODEL_SAFETY_NET].filter(function (name, index, arr) {
  return !!name && arr.indexOf(name) === index;
});
var GEMINI_MAX_RETRIES_PER_MODEL = 2;
var GEMINI_RETRY_BASE_DELAY_MS = 1500;
...
// Browsers sometimes report recorded audio blobs with a generic
// application/octet-stream content type instead of audio/webm - normalize
// and sniff the real format instead of rejecting the upload.
function baseMimeType(rawMimeType) {
  return (rawMimeType || '').split(';')[0].trim().toLowerCase();
}

function detectAudioMimeTypeFromBuffer(buffer) {
...
function resolveAudioMimeType(file) {
  var mimetype = baseMimeType(file.mimetype);
  var genericTypes = ['application/octet-stream', 'application/binary', ''];
  if (mimetype && genericTypes.indexOf(mimetype) === -1) {
    return mimetype;
  }

  var sniffed = detectAudioMimeTypeFromBuffer(file.buffer);
  if (sniffed) {
    return sniffed;
  }

  var name = (file.originalname || '').toLowerCase();
  if (name.indexOf('.wav') !== -1) return 'audio/wav';
  if (name.indexOf('.ogg') !== -1) return 'audio/ogg';
  if (name.indexOf('.m4a') !== -1 || name.indexOf('.mp4') !== -1) return 'audio/mp4';

  return 'audio/webm';
}

function callGeminiVoiceCommand(modelName, audioBuffer, mimeType) {
  return new Promise(function (resolve, reject) {
    if (!GEMINI_API_KEY) {
    ...
    var options = {
      ...
      path: '/v1beta/models/' + encodeURIComponent(modelName) + ':generateContent?key=' + encodeURIComponent(GEMINI_API_KEY),
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    var request = https.request(options, function (response) {
      ...
      response.on('end', function () {
        ...
        if (response.statusCode === 429 || response.statusCode === 503) {
          var busyErr = new Error('Voice command service is busy right now.');
          busyErr.statusCode = response.statusCode;
          reject(busyErr);
          return;
        }
...
// Production-grade resilience: walks the Pay-As-You-Go model chain
// (primary -> fallback -> proven safety-net alias), retrying each model up
// to GEMINI_MAX_RETRIES_PER_MODEL times with 1.5s exponential backoff for
// 429 (quota)/503 (overloaded) responses before moving to the next model.
// Non-retryable errors (bad model name, malformed response, etc.) skip
// straight to the next model without wasting retries.
async function callGeminiWithResilience(audioBuffer, mimeType) {
  var lastError = null;
  for (var modelIndex = 0; modelIndex < GEMINI_MODEL_CHAIN.length; modelIndex++) {
    var modelName = GEMINI_MODEL_CHAIN[modelIndex];
    for (var attempt = 0; attempt <= GEMINI_MAX_RETRIES_PER_MODEL; attempt++) {
      try {
        return await callGeminiVoiceCommand(modelName, audioBuffer, mimeType);
      } catch (err) {
        lastError = err;
        var isRetryable = err && (err.statusCode === 429 || err.statusCode === 503);
        if (isRetryable && attempt < GEMINI_MAX_RETRIES_PER_MODEL) {
          await delayMs(GEMINI_RETRY_BASE_DELAY_MS * Math.pow(2, attempt));
          continue;
        }
        break;
      }
  ...
  var finalErr = new Error('AI assistant is temporarily busy. Please try speaking your command again in a moment.');
  finalErr.statusCode = (lastError && lastError.statusCode) || 503;
  throw finalErr;
}

// Zero-latency local fast-path: matches common phrasing via keyword/regex
// against the browser's own live Web Speech transcript (sent as
// transcript_hint alongside the audio) so routine commands never have to
// wait on an external API round trip at all. Falls through (returns null)
// for anything ambiguous, letting Gemini handle nuance/Hindi-Hinglish/etc.
function tryLocalIntentMatch(transcriptRaw, activeFileExtension) {
  var transcript = String(transcriptRaw || '').trim().toLowerCase();
  if (!transcript) {
    return null;
  }
  var sourceExtension = String(activeFileExtension || '').trim().toLowerCase().replace(/^\./, '');

  var targetSizeKb = null;
  var kbMatch = transcript.match(/(\d+(?:\.\d+)?)\s*kb\b/);
  var mbMatch = transcript.match(/(\d+(?:\.\d+)?)\s*mb\b/);
  if (kbMatch) {
    targetSizeKb = Math.round(parseFloat(kbMatch[1]));
  } else if (mbMatch) {
    targetSizeKb = Math.round(parseFloat(mbMatch[1]) * 1024);
  }

  var presetKeywords = ['ssc', 'upsc', 'ibps', 'rrb', 'jee', 'neet', 'aadhaar', 'passport'];
  var matchedPreset = null;
  for (var i = 0; i < presetKeywords.length; i++) {
    if (transcript.indexOf(presetKeywords[i]) !== -1) {
      matchedPreset = presetKeywords[i];
      break;
    }
  }

  function buildResult(tool, action) {
    return {
      tool: tool,
      action: action,
      parameters: { target_size_kb: targetSizeKb, preset: matchedPreset },
      recognized_text: transcriptRaw,
      confidence: 0.92
    };
  }

  if (matchedPreset && /\b(ssc|upsc|ibps|rrb|jee|neet|exam|photo|signature)\b/.test(transcript)) {
    return buildResult('photo_resizer', 'resize');
  }

  if (/\b(protect|lock|encrypt|password)\b/.test(transcript)) {
    return buildResult('protect_pdf', 'protect');
  }

  if (/\b(merge|combine)\b/.test(transcript)) {
    return buildResult('merge_pdf', 'merge');
  }

  if (/\bsplit\b/.test(transcript)) {
    return buildResult('split_pdf', 'split');
  }
  if (/\bextract\b/.test(transcript)) {
    return buildResult('split_pdf', 'extract');
  }

  if (/\b(compress|reduce|shrink)\b/.test(transcript) || targetSizeKb !== null) {
    return buildResult('compress_pdf', 'compress');
  }

  if (/\bconvert\b/.test(transcript)) {
    ...
    var toIndex = transcript.indexOf(' to ');
    var targetPart = toIndex !== -1 ? transcript.substring(toIndex + 4) : transcript;

    var targetMentionsWord = /\b(word|docx|doc)\b/.test(targetPart);
    var targetMentionsExcel = /\b(excel|xlsx|xls|spreadsheet)\b/.test(targetPart);
    var targetMentionsCsv = /\bcsv\b/.test(targetPart);
    var targetMentionsImage = /\b(image|photo|jpg|jpeg|png)\b/.test(targetPart);
    var targetMentionsPdf = /\bpdf\b/.test(targetPart);
    var imageExtensions = ['jpg', 'jpeg', 'png', 'webp', 'bmp'];

    // STEP 2: context-aware routing matrix (source extension + spoken
    // target) - mirrors voice_command_service.dart's _tryLocalIntentMatch.
    ...
    if (sourceExtension && imageExtensions.indexOf(sourceExtension) !== -1) {
      // E. Image source.
    ...
    // fall back to guessing from the spoken target format alone.
    if (targetMentionsExcel || targetMentionsCsv) {
      if (/\bcsv\s+to\s+excel\b/.test(transcript)) {
    ...
    // No recognizable target format after "to" - let Gemini use the full
    // audio for extra nuance rather than guessing here.
    return null;
  }
...
var voiceCommandUpload = multer({
  ...
  fileFilter: function (req, file, cb) {
    var allowed = ['audio/webm', 'audio/ogg', 'audio/wav', 'audio/mp4', 'audio/mpeg', 'audio/x-m4a', 'audio/aac', 'application/octet-stream'];
    if (allowed.indexOf(baseMimeType(file.mimetype)) !== -1) {
      cb(null, true);
    } else {
...
// ---------------------------------------------------------------------------
// Gemini Flash AI Voice Mock Interview evaluation
// ---------------------------------------------------------------------------
function buildInterviewEvaluationPrompt(question, roleCategory) {
  return 'You are an expert, encouraging interview coach for a job-preparation platform called GetReadyJob. ' +
    'The candidate is practicing for a "' + roleCategory + '" role interview. ' +
    'The interview question asked was: "' + question + '". ' +
    'Listen to the attached short audio clip of the candidate\'s spoken answer in English or Hindi/Hinglish. ' +
    'Respond with ONLY a strict JSON object (no markdown, no code fences, no extra commentary) matching exactly this shape:\n' +
    '{"transcript": "<the transcribed words you heard>", ' +
    '"clarity": <integer 0-10, how clear and well-structured the speech was>, ' +
    '"confidence": <integer 0-10, how confident and steady the delivery sounded>, ' +
    '"content": <integer 0-10, how relevant and complete the answer content was for this specific question>, ' +
    '"feedback": "<one or two short, constructive, encouraging sentences of feedback>"}\n' +
    'Rules:\n' +
    '- If no meaningful speech is heard, set all three scores to 0 and feedback to a gentle prompt to try again.\n' +
    '- Scores must be whole numbers between 0 and 10.\n' +
    '- Output raw JSON only, nothing else.';
}

function callGeminiInterviewEvaluation(modelName, audioBuffer, mimeType, question, roleCategory) {
  return new Promise(function (resolve, reject) {
    if (!GEMINI_API_KEY) {
      var configErr = new Error('Voice interview evaluation is not configured on the server yet.');
      configErr.statusCode = 503;
      reject(configErr);
      return;
    }
    ...
    var requestBody = JSON.stringify({
      contents: [{
        parts: [
          { text: buildInterviewEvaluationPrompt(question, roleCategory) },
          { inlineData: { mimeType: mimeType, data: audioBuffer.toString('base64') } }
        ]
      }],
      generationConfig: {
    ...
    var request = https.request(options, function (response) {
      ...
      response.on('end', function () {
        ...
        if (response.statusCode === 429 || response.statusCode === 503) {
          var busyErr = new Error('Interview evaluation service is busy right now.');
          busyErr.statusCode = response.statusCode;
          reject(busyErr);
          return;
        }
        ...
        try {
          ...
          rawText = rawText.replace(/^```json\s*/i, '').replace(/^```\s*/, '').replace(/```\s*$/, '');
          var evaluation = JSON.parse(rawText);
          resolve(evaluation);
        } catch (err) {
          var understandErr = new Error('Could not evaluate that answer. Please try again.');
          understandErr.statusCode = 422;
          reject(understandErr);
        }
      });
    ...
    request.on('error', function () {
      var netErr = new Error('Could not reach the interview evaluation service. Please check your connection.');
      netErr.statusCode = 502;
      reject(netErr);
    });
...
// Mirrors callGeminiWithResilience's model-chain/retry strategy for the interview evaluator.
async function callGeminiInterviewEvaluationWithResilience(audioBuffer, mimeType, question, roleCategory) {
  var lastError = null;
  for (var modelIndex = 0; modelIndex < GEMINI_MODEL_CHAIN.length; modelIndex++) {
    var modelName = GEMINI_MODEL_CHAIN[modelIndex];
    for (var attempt = 0; attempt <= GEMINI_MAX_RETRIES_PER_MODEL; attempt++) {
      try {
        return await callGeminiInterviewEvaluation(modelName, audioBuffer, mimeType, question, roleCategory);
      } catch (err) {
        lastError = err;
        var isRetryable = err && (err.statusCode === 429 || err.statusCode === 503);
        if (isRetryable && attempt < GEMINI_MAX_RETRIES_PER_MODEL) {
  ...
  var finalErr = new Error('AI interview coach is temporarily busy. Please try answering again in a moment.');
  finalErr.statusCode = (lastError && lastError.statusCode) || 503;
  throw finalErr;
}
...
app.post('/api/voice-command', voiceCommandUpload.single('audio'), function (req, res) {
  ...
  function respondWithClassification(classification) {
    ...
    logAuditEvent('guest', 'voice-command', { tool: tool, confidence: confidence, matchedLocally: !!classification.matchedLocally });
    ...
    res.json({
      ...
      matchedLocally: !!classification.matchedLocally
    });
  }

  // Zero-latency fast path: the browser's own live Web Speech transcript
  // (if it produced one) is matched locally first - no external API call,
  // no network latency, works even if Gemini is fully down.
  var transcriptHint = String((req.body && req.body.transcript_hint) || '').trim();
  var activeFileExtension = String((req.body && req.body.active_file_extension) || '').trim();
  var localMatch = transcriptHint ? tryLocalIntentMatch(transcriptHint, activeFileExtension) : null;
  if (localMatch) {
    localMatch.matchedLocally = true;
    return respondWithClassification(localMatch);
  }

  var resolvedMimeType = resolveAudioMimeType(req.file);
  callGeminiWithResilience(req.file.buffer, resolvedMimeType).then(function (classification) {
    respondWithClassification(classification);
  }).catch(function (err) {
    var status = (err && err.statusCode) || 500;
    res.status(status).json({ success: false, error: (err && err.message) || 'AI assistant is temporarily busy. Please try speaking your command again in a moment.' });
  });
});

app.post('/api/voice-interview-evaluate', voiceCommandUpload.single('audio'), function (req, res) {
  if (!req.file) {
    return res.status(400).json({ success: false, error: 'No audio recorded.' });
  }
  ...
  if (!question) {
    return res.status(400).json({ success: false, error: 'Missing interview question context.' });
  }
  var roleCategory = String((req.body && req.body.roleCategory) || 'General').trim() || 'General';
  ...
  var resolvedMimeType = resolveAudioMimeType(req.file);
  callGeminiInterviewEvaluationWithResilience(req.file.buffer, resolvedMimeType, question, roleCategory).then(function (evaluation) {
    var clarity = clampScore(evaluation && evaluation.clarity);
    var confidence = clampScore(evaluation && evaluation.confidence);
    var content = clampScore(evaluation && evaluation.content);
    var overallScore = Math.round(((clarity + confidence + content) / 3) * 10) / 10;

    logAuditEvent('guest', 'voice-interview-evaluate', { roleCategory: roleCategory, overallScore: overallScore });

    res.json({
      success: true,
      transcript: String((evaluation && evaluation.transcript) || '').trim(),
      clarity: clarity,
      confidence: confidence,
      content: content,
      overallScore: overallScore,
      feedback: String((evaluation && evaluation.feedback) || '').trim() || 'Good attempt - keep practicing for more confident, detailed delivery.'
    });
  }).catch(function (err) {
    var status = (err && err.statusCode) || 500;
    res.status(status).json({ success: false, error: (err && err.message) || 'AI interview coach is temporarily busy. Please try answering again in a moment.' });
  });
});
...
app.post('/api/compress', upload.single('file'), enforceQuotaMiddleware, function (req, res) {
  ...
  try {
    ...
    // Optional explicit target size (bytes) for multi-pass PDF compression.
    var targetBytes = parseInt(req.body.targetBytes, 10);
    ...
    var resolvedFileMimeType = req.file.resolvedMimeType || req.file.mimetype;
    var isImage = resolvedFileMimeType.startsWith('image/');
    var isPdf = resolvedFileMimeType === 'application/pdf';
    ...
    if (isImage && targetBytes) {
      console.log('→ Executing: compressImageToTarget (multi-pass, target=' + targetBytes + ' bytes)');
      compressImageToTarget(tempInputPath, tempOutputPath, format, targetBytes, done);
    } else if (isImage) {
      console.log('→ Executing: compressImage (image file)');
      compressImage(tempInputPath, tempOutputPath, quality, format, done);
    } else if (isPdf && targetBytes) {
      console.log('→ Executing: compressPdfToTarget (multi-pass, target=' + targetBytes + ' bytes)');
      compressPdfToTarget(tempInputPath, tempOutputPath, targetBytes, done);
...
// Photo HD Workspace - server-side (sharp/libvips) preset rendering for
// large canvases (A2/A3 posters, 2K+) that would otherwise lag the browser's
// UI thread if rendered client-side. Client-side quota/plan access for the
// HD Photo Studio tool is already enforced before this is ever called, so
// this endpoint intentionally does NOT run enforceQuotaMiddleware again -
// it is purely a compute offload for a tool the caller already has access to.
app.post('/api/photo/render-preset', upload.single('image'), function (req, res) {
  var tempInputPath = null;
  var tempOutputPath = null;
  ...
  try {
    ...
    if (!width || !height || width <= 0 || height <= 0) {
      cleanupFile(req.file.path);
      return res.status(400).json({ success: false, error: 'Invalid target width/height' });
    }

    var outputFormat = (req.body.outputFormat || 'jpg').toLowerCase();
    if (['jpg', 'png', 'pdf'].indexOf(outputFormat) === -1) {
      cleanupFile(req.file.path);
      return res.status(400).json({ success: false, error: 'outputFormat must be jpg, png, or pdf' });
    }

    tempInputPath = persistUploadedFile(req.file, req) || req.file.path;
    var baseName = path.parse(req.file.originalname).name.substring(0, 50);
    var outputExt = outputFormat === 'pdf' ? '.pdf' : (outputFormat === 'png' ? '.png' : '.jpg');
    tempOutputPath = path.join(UPLOAD_DIR, baseName + '_preset_' + Date.now() + outputExt);
    ...
    var timer = setTimeout(function () {
      ...
      cleanupFile(tempInputPath);
      cleanupFile(tempOutputPath);
      res.status(408).json({ success: false, error: 'Rendering took too long. Try again or use a smaller preset.' });
    }, 60 * 1000);
    ...
    var renderOptions = {
      ...
      enableHdMode: req.body.enableHdMode === 'true',
      outputFormat: outputFormat,
      dpi: parseInt(req.body.dpi, 10) || 300,
      maxTargetKb: parseInt(req.body.maxTargetKb, 10) || 0,
      enforceFileSizeLimit: req.body.enforceFileSizeLimit === 'true'
    };

    renderPhotoPresetWithSharp(tempInputPath, tempOutputPath, renderOptions, function (err) {
      if (responded) return;
      clearTimeout(timer);

      if (err) {
        responded = true;
        console.error('[photo-render-preset] Failed: ' + (err.message || err));
        cleanupFile(tempInputPath);
        cleanupFile(tempOutputPath);
        return res.status(500).json({ success: false, error: err.message || 'Photo rendering failed' });
      }
      ...
      if (fileSize(tempOutputPath) === 0) {
        ...
        cleanupFile(tempInputPath);
        cleanupFile(tempOutputPath);
        return res.status(500).json({ success: false, error: 'Rendering produced no output' });
      }

      responded = true;
      var downloadName = baseName + '_rendered' + outputExt;
      res.set('X-Render-Engine', 'sharp-server');
      res.download(tempOutputPath, downloadName, function (dlErr) {
        cleanupFile(tempInputPath);
        cleanupFile(tempOutputPath);
        if (dlErr) console.error('[photo-render-preset] Download error:', dlErr.message);
      });
    });
  } catch (e) {
    cleanupFile(tempInputPath);
...
// HD Photo Studio - "Remove Background / Transparent Cutout". Same quota
// model as /api/photo/render-preset above (client-side gated, no
// enforceQuotaMiddleware here) - purely a compute offload for a tool the
// caller already has access to.
app.post('/api/photo/remove-bg', upload.single('image'), function (req, res) {
  var tempInputPath = null;
  var tempOutputPath = null;
  ...
  try {
    ...
    tempInputPath = persistUploadedFile(req.file, req) || req.file.path;
    var baseName = path.parse(req.file.originalname).name.substring(0, 50);
    tempOutputPath = path.join(UPLOAD_DIR, baseName + '_nobg_' + Date.now() + '.png');
    ...
    var timer = setTimeout(function () {
      ...
      cleanupFile(tempInputPath);
      cleanupFile(tempOutputPath);
      res.status(408).json({ success: false, error: 'Background removal took too long. Try a smaller image.' });
    }, 30 * 1000);

    removeImageBackground(tempInputPath, tempOutputPath, function (err) {
      if (responded) return;
      clearTimeout(timer);
      responded = true;
      cleanupFile(tempInputPath);
      ...
      if (err) {
        console.error('[photo-remove-bg] Failed: ' + err.message);
        cleanupFile(tempOutputPath);
        return res.status(500).json({ success: false, error: err.message || 'Background removal failed' });
      }

      if (fileSize(tempOutputPath) === 0) {
        cleanupFile(tempOutputPath);
        return res.status(500).json({ success: false, error: 'Background removal produced no output' });
      }

      res.set('Content-Type', 'image/png');
      res.set('X-BgRemoval-Engine', 'sharp-color-distance');
      res.download(tempOutputPath, baseName + '_transparent.png', function (dlErr) {
        cleanupFile(tempOutputPath);
        if (dlErr) console.error('[photo-remove-bg] Download error:', dlErr.message);
      });
    });
  } catch (e) {
    cleanupFile(tempInputPath);
...
// High-fidelity PDF -> DOCX conversion (structural layout via LibreOffice).
// Returns a clear error on any failure so the client can fall back to its
// own local text-extraction pipeline - never silently degrades quality.
app.post('/api/convert-pdf-to-docx', upload.single('file'), enforceQuotaMiddleware, function (req, res) {
  var tempInputPath = null;
  recordToolUsageEvent(req.body && req.body.planId ? req.body.planId : '', (req.body && req.body.tool) ? req.body.tool : 'convert', { source: 'convert-pdf-to-docx-endpoint' });

  // Keeps the underlying TCP/proxy connection alive for the full duration of
  // this potentially long-running (multi-tier fallback) conversion, instead
  // of the proxy assuming the connection is idle/dead and dropping it early.
  ...
  try {
    ...
    if ((req.file.resolvedMimeType || req.file.mimetype) !== 'application/pdf') {
      cleanupFile(req.file.path);
      return res.status(400).json({ success: false, error: 'Only PDF files are supported for this endpoint.' });
    }
    ...
    var timer = setTimeout(function () {
      responded = true;
      cleanupFile(tempInputPath);
      res.status(408).json({ success: false, error: 'PDF to Word conversion took too long. Try a smaller or simpler file.' });
    }, 340 * 1000);

    convertPdfToDocxHighFidelity(tempInputPath, function (err, docxBytes, engine) {
      if (responded) return;
      clearTimeout(timer);
      responded = true;
      cleanupFile(tempInputPath);
      ...
      if (err) {
        console.error('[convert-pdf-to-docx] Conversion failed for "' + (req.file && req.file.originalname) + '": ' + (err.message || err));

        if (err.isScanned) {
          return res.status(422).json({
            success: false,
            isScanned: true,
            message: 'This document is a scanned image PDF with no selectable text. Use OCR or Extract Images tool.'
          });
        }

        var status = 500;
        var msg = err.message || 'PDF to Word conversion failed';
        if (msg.indexOf('timeout') !== -1 || msg.indexOf('ETIMEDOUT') !== -1) status = 408;
        else if (msg.indexOf('Invalid') !== -1) status = 400;
        return res.status(status).json({ success: false, error: msg });
      }

      var consumption = consumePlanQuota(req.quotaContext && req.quotaContext.planId ? req.quotaContext.planId : '', req.quotaContext && req.quotaContext.tool ? req.quotaContext.tool : 'convert', true);
      logAuditEvent(req.body && req.body.planId ? req.body.planId : 'guest', 'tool-convert-pdf-docx', { tool: 'convert', planId: req.quotaContext && req.quotaContext.planId ? req.quotaContext.planId : '' });

      res.set('Content-Type', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
      res.set('Content-Disposition', 'attachment; filename="' + baseName + '.docx"');
      res.set('X-Conversion-Engine', engine || 'unknown');
      res.set('X-Quota-Remaining', String(consumption.remainingQuota === null ? '' : consumption.remainingQuota));
      res.set('X-Quota-Used', String(consumption.used));
      res.set('X-Quota-Limit', String(consumption.limit === null ? '' : consumption.limit));
      res.set('X-Quota-Exhausted', String(consumption.exhausted));
    ...
    console.error('[convert-pdf-to-docx] Unhandled exception in route handler: ' + (e && e.stack ? e.stack : e));
    cleanupFile(tempInputPath);
    res.status(500).json({ success: false, error: e.message || 'Internal server error' });
  }
});

// Pure-Node page-count fallback (pdf-lib, already a proven dependency in
// this file) - used when the Python/PyMuPDF text-presence check fails open
// with no page count (e.g. python3 unresolvable at runtime, mirroring the
// earlier soffice ENOENT issue), so a broken Python environment never blocks
// the OCR routes entirely.
function getPdfPageCountViaPdfLib(inputPath, callback) {
  var PDFDocument;
  try {
...
// Shared handler for the two Google Cloud Vision OCR routes below - both
// validate the upload, check the PDF's page count, reserve pages against the
// global monthly cap, run the OCR orchestration function, then stream back
// the result with usage headers. Neither route runs enforceQuotaMiddleware -
// per-plan OCR quota is enforced client-side (ocr_quota_service.dart); the
// global monthly cap (reserveOcrGlobalPages) is this route's only server-
// enforced protection against runaway Vision API cost.
function handleSharedOcrRoute(req, res, options) {
  var tempInputPath = null;
  var tempOutputPath = null;
  ...
  try {
    ...
    tempInputPath = persistUploadedFile(req.file, req) || req.file.path;
    var baseName = path.parse(req.file.originalname).name.substring(0, 50);
    tempOutputPath = path.join(UPLOAD_DIR, baseName + '_ocr_' + Date.now() + options.outputExt);

    function proceedWithPageCount(pageCount) {
      var reservation = reserveOcrGlobalPages(pageCount);
      if (!reservation.allowed) {
        cleanupFile(tempInputPath);
        return res.status(429).json({
          ...
          message: 'Monthly AI OCR system limit reached. Resets on the 1st of next month.'
        });
      }
      ...
      var timer = setTimeout(function () {
        ...
        cleanupFile(tempInputPath);
        cleanupFile(tempOutputPath);
        res.status(408).json({ success: false, error: 'OCR took too long. Try a smaller or simpler file.' });
      }, 170 * 1000);

      options.runOcr(tempInputPath, tempOutputPath, function (err, result) {
        if (responded) return;
        clearTimeout(timer);
        responded = true;
        cleanupFile(tempInputPath);
        ...
        if (err) {
          console.error('[' + options.logPrefix + '] Failed: ' + err.message);
          cleanupFile(tempOutputPath);
          var status = err.message && err.message.indexOf('not configured') !== -1 ? 503 : 500;
          return res.status(status).json({ success: false, error: err.message || 'OCR conversion failed' });
        }

        if (fileSize(tempOutputPath) === 0) {
          cleanupFile(tempOutputPath);
          return res.status(500).json({ success: false, error: 'OCR produced no output' });
        }

        var downloadName = baseName + options.downloadSuffix + options.outputExt;
        res.set('Content-Type', options.contentType);
        res.set('X-Ocr-Pages-Used', String(result.pageCount));
        res.set('X-Ocr-Global-Remaining', String(Math.max(0, OCR_GLOBAL_MONTHLY_PAGE_CAP - reservation.used)));
        res.download(tempOutputPath, downloadName, function (dlErr) {
          cleanupFile(tempOutputPath);
          if (dlErr) console.error('[' + options.logPrefix + '] Download error:', dlErr.message);
        });
      });
    }

    checkPdfHasSelectableText(tempInputPath, function (textInfo) {
      var pageCount = (textInfo && textInfo.pageCount) || 0;
      if (pageCount) {
        return proceedWithPageCount(pageCount);
      }

      // Python check didn't yield a page count (checkFailed, or genuinely
      // 0 pages) - fall back to pdf-lib before giving up entirely.
      getPdfPageCountViaPdfLib(tempInputPath, function (countErr, fallbackCount) {
        if (countErr || !fallbackCount) {
          console.error('[' + options.logPrefix + '] pdf-lib page-count fallback also failed: ' + (countErr ? countErr.message : 'no pages'));
          cleanupFile(tempInputPath);
          return res.status(400).json({ success: false, error: "Could not determine this PDF's page count." });
        }
        proceedWithPageCount(fallbackCount);
      });
    });
  } catch (e) {
    cleanupFile(tempInputPath);
...
// Scanned PDF -> Searchable PDF via Google Cloud Vision OCR ("sandwich" PDF:
// original page images + an invisible, positioned, selectable text layer).
app.post('/api/ocr-pdf', upload.single('file'), function (req, res) {
  handleSharedOcrRoute(req, res, {
    outputExt: '.pdf',
    contentType: 'application/pdf',
    downloadSuffix: '_searchable',
    logPrefix: 'ocr-pdf',
    runOcr: ocrScannedPdfToSearchablePdf
  });
});

// Scanned PDF -> OCR -> plain-text DOCX via the same shared Google Cloud
// Vision engine as /api/ocr-pdf above.
app.post('/api/convert-scanned-pdf-to-docx', upload.single('file'), function (req, res) {
  handleSharedOcrRoute(req, res, {
    outputExt: '.docx',
    contentType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
    downloadSuffix: '_ocr',
    logPrefix: 'convert-scanned-pdf-to-docx',
    runOcr: ocrScannedPdfToDocx
  });
});

// Generic conversion endpoint - PDF -> Image (JPG/PNG) pages as a ZIP
// archive (rasterized via Ghostscript), or spreadsheet conversions (XLSX/
// XLS/CSV) via headless LibreOffice (and Python/openpyxl for multi-sheet
// XLSX -> CSV) - all server-side so large/complex files never need to be
// parsed or rendered in browser memory.
app.post('/api/convert', upload.single('file'), enforceQuotaMiddleware, function (req, res) {
  var tempInputPath = null;
  recordToolUsageEvent(req.body && req.body.planId ? req.body.planId : '', (req.body && req.body.tool) ? req.body.tool : 'convert', { source: 'convert-endpoint' });
  ...
  try {
    ...
    var resolvedFileType = req.file.resolvedMimeType || req.file.mimetype;
    var isXlsxInput = resolvedFileType === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
    var isXlsInput = resolvedFileType === 'application/vnd.ms-excel';
    var isCsvInput = resolvedFileType === 'text/csv';
    var isSpreadsheetInput = isXlsxInput || isXlsInput || isCsvInput;

    if (isSpreadsheetInput) {
      var allowedSheetTargets = isCsvInput ? ['xlsx', 'pdf'] : ['csv', 'pdf'];
      var sheetTargetFormat = (req.body.targetFormat || allowedSheetTargets[0]).toLowerCase();
      if (allowedSheetTargets.indexOf(sheetTargetFormat) === -1) {
        cleanupFile(req.file.path);
        return res.status(400).json({ success: false, error: 'targetFormat must be ' + allowedSheetTargets.join(' or ') + ' for this input type' });
      }

      tempInputPath = persistUploadedFile(req.file, req) || req.file.path;
      var sheetBaseName = path.parse(req.file.originalname).name.substring(0, 50);

      var sheetResponded = false;
      var sheetTimer = setTimeout(function () {
        sheetResponded = true;
        cleanupFile(tempInputPath);
        res.status(408).json({ success: false, error: 'Conversion took too long. Try a smaller or simpler file.' });
      }, 150 * 1000);

      function finishSpreadsheet(err, outputBytes, contentType, downloadName) {
        if (sheetResponded) return;
        ...
        if (err) {
          var sheetStatus = 500;
          var sheetMsg = err.message || 'Conversion failed';
          if (sheetMsg.indexOf('timeout') !== -1) sheetStatus = 408;
          else if (sheetMsg.indexOf('Invalid') !== -1) sheetStatus = 400;
          return res.status(sheetStatus).json({ success: false, error: sheetMsg });
        }

        var sheetConsumption = consumePlanQuota(req.quotaContext && req.quotaContext.planId ? req.quotaContext.planId : '', req.quotaContext && req.quotaContext.tool ? req.quotaContext.tool : 'convert', true);
        logAuditEvent(req.body && req.body.planId ? req.body.planId : 'guest', 'tool-convert-spreadsheet-' + sheetTargetFormat, { tool: 'convert', planId: req.quotaContext && req.quotaContext.planId ? req.quotaContext.planId : '' });

        res.set('Content-Type', contentType);
        res.set('Content-Disposition', 'attachment; filename="' + downloadName + '"');
        res.set('X-Quota-Remaining', String(sheetConsumption.remainingQuota === null ? '' : sheetConsumption.remainingQuota));
        res.set('X-Quota-Used', String(sheetConsumption.used));
        res.set('X-Quota-Limit', String(sheetConsumption.limit === null ? '' : sheetConsumption.limit));
        res.set('X-Quota-Exhausted', String(sheetConsumption.exhausted));
        res.send(outputBytes);
      }

      if (isXlsxInput && sheetTargetFormat === 'csv') {
        // Multi-sheet-aware export: every sheet becomes its own CSV, bundled
        // into a ZIP when there is more than one - never silently drops
        // extra sheets the way a single soffice --convert-to csv call would.
        convertXlsxToCsvPerSheet(tempInputPath, function (xlsxErr, result) {
          if (xlsxErr) return finishSpreadsheet(xlsxErr);

          if (result.sheetFiles.length === 1) {
            var singleCsvBytes;
            try {
              singleCsvBytes = fs.readFileSync(result.sheetFiles[0]);
            } catch (readErr) {
              removeWorkDir(result.outDir);
              return finishSpreadsheet(new Error('Cannot read exported CSV: ' + readErr.message));
            }
            removeWorkDir(result.outDir);
            return finishSpreadsheet(null, singleCsvBytes, 'text/csv', sheetBaseName + '.csv');
          }

          zipFilesToBuffer(result.sheetFiles, function (zipErr, zipBytes) {
            removeWorkDir(result.outDir);
            if (zipErr) return finishSpreadsheet(zipErr);
            finishSpreadsheet(null, zipBytes, 'application/zip', sheetBaseName + '_sheets.zip');
          });
        });
        return;
      }

      convertSpreadsheetWithLibreOffice(tempInputPath, sheetTargetFormat, function (err, outputBytes) {
        var contentType = sheetTargetFormat === 'pdf'
          ? 'application/pdf'
          : sheetTargetFormat === 'xlsx'
            ? 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
            : 'text/csv';
        finishSpreadsheet(err, outputBytes, contentType, sheetBaseName + '.' + sheetTargetFormat);
      });
      return;
    }
    ...
    if (resolvedFileType !== 'application/pdf') {
      cleanupFile(req.file.path);
      return res.status(400).json({ success: false, error: 'Only PDF, Excel (XLSX/XLS), or CSV files are supported for this endpoint.' });
    }

    var targetFormat = (req.body.targetFormat || 'jpg').toLowerCase();
    if (targetFormat === 'jpeg') targetFormat = 'jpg';
    if (['jpg', 'png'].indexOf(targetFormat) === -1) {
      cleanupFile(req.file.path);
      return res.status(400).json({ success: false, error: 'targetFormat must be jpg or png' });
    }
    ...
    convertPdfToImagesArchive(tempInputPath, targetFormat, function (err, zipBytes) {
      if (responded) return;
      clearTimeout(timer);
      responded = true;
      cleanupFile(tempInputPath);
      ...
      var consumption = consumePlanQuota(req.quotaContext && req.quotaContext.planId ? req.quotaContext.planId : '', req.quotaContext && req.quotaContext.tool ? req.quotaContext.tool : 'convert', true);
      logAuditEvent(req.body && req.body.planId ? req.body.planId : 'guest', 'tool-convert-pdf-images', { tool: 'convert', planId: req.quotaContext && req.quotaContext.planId ? req.quotaContext.planId : '' });

      res.set('Content-Type', 'application/zip');
      res.set('Content-Disposition', 'attachment; filename="' + baseName + '_' + targetFormat + '_pages.zip"');
      res.set('X-Quota-Remaining', String(consumption.remainingQuota === null ? '' : consumption.remainingQuota));
      res.set('X-Quota-Used', String(consumption.used));
      res.set('X-Quota-Limit', String(consumption.limit === null ? '' : consumption.limit));
      res.set('X-Quota-Exhausted', String(consumption.exhausted));
...
if (require.main === module) {
  // -------------------------------------------------------------------------
  // Self-healing / auto-recovery shield. Registered ONLY when this file runs
  // as the actual server (not when `test/*.test.js` `require()`s `app` for
  // supertest) so an unrelated test-time rejection can never trigger this.
  //
  // Node's own docs warn that continuing to run after an `uncaughtException`
  // leaves the process in an undefined state - so this does NOT try to keep
  // limping along forever. A single isolated error is logged and swallowed
  // (one bad request shouldn't take the whole server down for every user),
  // but if critical errors keep piling up within a rolling 5-minute window
  // (signal of a deeper problem - leak, stuck handle, corrupted state), it
  // logs an explicit auto-repair notice and performs a CONTROLLED restart -
  // the only genuinely complete way to reset all in-memory state. Render
  // (this app's host) auto-restarts the container within seconds, so this is
  // real self-healing, not a crash.
  // -------------------------------------------------------------------------
  var CRASH_GUARD_WINDOW_MS = 5 * 60 * 1000;
  var CRASH_GUARD_THRESHOLD = 5;
  var recentCriticalErrors = [];
  var isRecycling = false;

  function recordCriticalErrorAndMaybeRecycle(label, err) {
    ...
    var now = Date.now();
    recentCriticalErrors.push(now);
    recentCriticalErrors = recentCriticalErrors.filter(function (ts) { return now - ts <= CRASH_GUARD_WINDOW_MS; });

    if (recentCriticalErrors.length > CRASH_GUARD_THRESHOLD) {
      isRecycling = true;
      console.error(
        '[CRITICAL AUTO-REPAIR] Memory flush & pool recycling triggered - ' +
        recentCriticalErrors.length + ' critical errors in the last 5 minutes. Restarting for a clean recovery.'
      );
      if (typeof global.gc === 'function') {
        try { global.gc(); } catch (gcErr) { /* --expose-gc not enabled, ignore */ }
      }
      // Brief delay lets the log line above (and any in-flight response)
      // flush before exit; Render detects the exit and restarts the
  ...
  process.on('uncaughtException', function (err) {
    recordCriticalErrorAndMaybeRecycle('UNCAUGHT EXCEPTION', err);
  });
  process.on('unhandledRejection', function (reason) {
    recordCriticalErrorAndMaybeRecycle('UNHANDLED REJECTION', reason);
  });
  ...
  app.listen(PORT, function () {
    ...
    console.log('  pdf2docx (Tier 1 PDF-to-Word primary engine): ' + (isPdf2docxAvailable() ? 'FOUND at ' + resolvePdf2docxBinary() : "NOT FOUND - every conversion will silently fall back to Tier 2/3, losing pdf2docx's superior table/column fidelity"));
    console.log('  LibreOffice (soffice): ' + (isLibreOfficeAvailable() ? 'FOUND at ' + resolveLibreOfficeBinary() : 'NOT FOUND - PDF-to-Word Tier 2 fallback and spreadsheet conversion will be unavailable'));
    console.log('  Google Vision OCR (Tier 3 PDF-to-Word fallback): ' + (resolveGoogleVisionApiKey() ? 'CONFIGURED' : 'NOT CONFIGURED - set GOOGLE_CLOUD_VISION_API_KEY to enable'));
    console.log('===========================================');
    console.log('');
  });
}
