...
// a JPEG via Ghostscript, builds a minimal HTML document (one image per page,
// forced page breaks), then converts THAT to DOCX via LibreOffice - so the
// user always gets a valid, openable Word document (page images, not
// searchable text, since there is no text to recover from a pure scan)
// instead of a hard failure. Every path cleans up its own temp directories.
// ---------------------------------------------------------------------------
function convertPdfToDocxFromRasterizedImages(inputPath, callback) {
  ...
  var profileDir = path.join(UPLOAD_DIR, 'p2drasf_profile_' + jobId);
  ...
  execFile(gsCommand, gsArgs, { timeout: 80000, maxBuffer: 20 * 1024 * 1024 }, function (gsErr) {
    if (gsErr) {
      cleanupAll();
    ...
    var htmlPath = path.join(pageDir, 'document.html');
    var htmlParts = ['<!DOCTYPE html><html><head><meta charset="utf-8"></head><body>'];
    pageFiles.forEach(function (fileName, index) {
      var isLast = index === pageFiles.length - 1;
      htmlParts.push(
        '<div style="' + (isLast ? '' : 'page-break-after: always;') + '">' +
        '<img src="' + fileName + '" style="width:100%;max-width:700px;display:block;margin:0 auto;">' +
        '</div>'
      );
    });
    ...
    try {
      ...
      cleanupAll();
      return callback(new Error('Cannot prepare fallback document: ' + writeErr.message));
    }
    ...
    acquireLibreOfficeSlot(function () {
      ...
      execFile(sofficeBin, args, { timeout: 100000, maxBuffer: 20 * 1024 * 1024 }, function (loErr) {
        releaseLibreOfficeSlot();

        if (loErr) {
          cleanupAll();
          return callback(new Error('Fallback image-based DOCX conversion failed: ' + loErr.message));
        }
...
function convertPdfToDocxHighFidelity(inputPath, callback) {
  convertPdfToDocxWithPdf2docx(inputPath, function (pdf2docxErr, docxBytes) {
    ...
    console.warn('[convert-pdf-to-docx] pdf2docx unavailable/failed, falling back to LibreOffice: ' + (pdf2docxErr ? pdf2docxErr.message : 'unknown error'));
    ...
    convertPdfToDocxWithLibreOffice(inputPath, function (loErr, loDocxBytes) {
      ...
      console.warn('[convert-pdf-to-docx] LibreOffice also unavailable/failed, falling back to rasterized-image DOCX: ' + (loErr ? loErr.message : 'unknown error'));
