...
// break between source pages. Unlike the removed Tier 3 image-embedding
// hack, this is a normal, non-fragile use of a hand-built OOXML package
// (plain paragraph text only), mirroring the same archiver settings already
// used elsewhere in this file (zipFilesToBuffer).
function buildSimpleDocxFromPages(pageTexts, outputPath, callback) {
  var archiver;
  try { archiver = require('archiver'); } catch (e) {
    return callback(new Error('archiver module not available. Re-run npm install'));
  }

  var bodyParts = pageTexts.map(function (pageText, index) {
    var lines = pageText.split(/\r?\n/).map(function (line) { return line.trim(); }).filter(function (line) { return line.length > 0; });
    if (lines.length === 0) {
      lines = ['(No text detected on this page.)'];
    }
    var paragraphsXml = lines.map(function (line) {
      return '<w:p><w:r><w:t xml:space="preserve">' + escapeXml(line) + '</w:t></w:r></w:p>';
    }).join('');
    var isLast = index === pageTexts.length - 1;
...
// Orchestrates: rasterize -> Vision OCR -> plain-text DOCX.
function ocrScannedPdfToDocx(inputPath, outputPath, callback) {
  rasterizePdfPagesForOcr(inputPath, function (rasterErr, rasterResult) {
    ...
    runVisionOcrOnPages(rasterResult.pages, function (ocrErr, ocrPages) {
      ...
      var pageTexts = ocrPages.map(function (page) { return page.text || ''; });
      buildSimpleDocxFromPages(pageTexts, outputPath, function (buildErr) {
        if (buildErr) return callback(buildErr);
        callback(null, { pageCount: rasterResult.pages.length });
      });
    });
