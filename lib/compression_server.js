...
// Builds one <w:p> for a single visual line. A large horizontal gap between
// consecutive words (> 35% of page width) is treated as a right-margin-
// anchored segment (marks/dates in exam papers) via a right tab stop rather
// than letting it wrap onto its own line. A single, roughly page-centered
// segment gets center alignment (headings/titles). Bold is applied when the
// line's average word height is notably above the page's average, or it
...
function buildLineParagraphXml(lineWords, pageWidthPx, avgWordHeight) {
  ...
  var segmentTexts = segments
    .map(function (segment) {
      ...
      // Clean stray spaces around brackets and punctuation
      lineText = lineText
        .replace(/\(\s+/g, '(')
        .replace(/\s+\)/g, ')')
        .replace(/\s+([.,!?:;])/g, '$1');
  ...
  var isCentered = pageWidthPx > 0 && Math.abs(lineCenter - pageCenter) <= pageWidthPx * 0.06;
  var centerPPr = isCentered ? '<w:pPr><w:jc w:val="center"/></w:pPr>' : '';

  return '<w:p>' + centerPPr + buildRunXml(segmentTexts[0], bold) + '</w:p>';
}

// Builds every paragraph for one page. Falls back to plain per-line text
// (no spatial reconstruction) if this page has no word-level boxes at all -
// keeps the output valid/non-empty even if Vision returned only flat text.
function buildPageParagraphsXml(ocrPage, pageWidthPx) {
  ...
  return lines.map(function (lineWords) { return buildLineParagraphXml(lineWords, pageWidthPx, avgWordHeight); }).join('');
}
