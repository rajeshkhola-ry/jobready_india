...
- Root cause found:
  ...
  - `jobready-india-docker.onrender.com` (the live backend behind getreadyjob.com) is `lib/compression_server.js`
...
- Overall status: Green (deployed)
- Completed:
  - **Root cause #1 — wrong backend domain** ✓ — `_VoiceTopupPacksSectionState._requestJson` had a hardcoded `https://getreadyjob.onrender.com` base URL, a completely different domain from the app's real backend (`ApiConfig.baseUrl` → `https://jobready-india-docker.onrender.com`) used everywhere else in the file. Every voice top-up call (`/api/config`, `/api/voice-topup/create-order`, `/api/voice-topup/verify`) was silently going to the wrong host, guaranteeing failure - this was the actual underlying error still firing even after the previous checkpoint's `_safeErrorText` sanitization started hiding the raw "Instance of 'minified:xx'" text. Fixed to use `ApiConfig.baseUrl`.
...
### Same day follow-up — PDF-to-Word module closed: transparent fidelity guidance added
- Overall status: Green (deployed and verified)
- Context: founder decided to stop investing further cycles/paid enterprise OCR APIs into reconstructing complex nested tables from scanned documents (the open-source Vision-OCR fallback already extracts scanned text accurately, just not complex nested table geometry) - digital-PDF high-fidelity routing (pdf2docx first, confirmed correct in the prior two same-day checkpoints) stays untouched.
- Completed:
  - **New fidelity tip added** ✓ — `Pages/convert_tool_page.dart`'s conversion UI now shows a small info-styled banner, visible specifically when the current selection is PDF → Word (.docx): "Tip: Fully preserves formatting for digital PDFs. Scanned/stamped forms convert as editable text." Placed directly under the existing "Supported: ..." hint line, matching this file's existing hint-banner visual language.
  - **No backend/engine changes this checkpoint** - per the founder's explicit decision to close this module, only the UI transparency note was added; the routing/fidelity work from the prior two checkpoints (pdf2docx-first routing confirmed correct, verification-screen fidelity-preservation fix, pdf2docx binary hardening + diagnostics) stands as the final state of the PDF-to-Word pipeline.
  - **Validation** ✓ — `flutter analyze lib/Pages/convert_tool_page.dart` → 0 errors (13 pre-existing, unrelated issues: `withOpacity` deprecation infos, one unrelated `unused_element` warning for `_startConversion`, one `unnecessary_to_list_in_spreads` info). Full-project `flutter analyze` → 312 issues, exact match to the documented baseline (0 new). `flutter build web --release -t lib/main_v1_1.dart --base-href / --no-wasm-dry-run --no-tree-shake-icons` and `firebase deploy --only hosting --project getreadyjob-india-1cb34` both succeeded.
  - **Git note**: staged only `lib/Pages/convert_tool_page.dart` plus `build_release_log.txt`/`firebase_deploy_log.txt`, rather than the literal `git add -A` the request specified — `lib/Widgets/upload_card_v2.dart`'s standing, deliberately-uncommitted local-only change was excluded again.
- Owner: Founder + Copilot
