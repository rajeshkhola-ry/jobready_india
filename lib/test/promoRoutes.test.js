...
// Isolate promo-code state too - otherwise usedCount/active from a previous test run
// persists on disk and leaks into this run's "must not mutate on preview" assertions.
process.env.PROMO_CODES_STATE_FILE = path.join(os.tmpdir(), 'grj-promos-test-' + process.pid + '-' + Date.now() + '.json');
...
// pdf-lib Flate-compresses content streams, so raw byte matching no longer works.
// Inflate every stream and decode the Tj/TJ text-showing operators back to plain text.
function extractPdfText(buffer) {
  ...
  while ((match = streamRe.exec(raw)) !== null) {
    try {
      ...
      // not a Flate-compressed stream (e.g. font/xref data); ignore
    }
  }
  let text = '';
  const opRe = /<([0-9A-Fa-f]+)>\s*Tj|\(((?:\\.|[^()])*)\)\s*Tj/g;
  let opMatch;
  while ((opMatch = opRe.exec(decoded)) !== null) {
    if (opMatch[1]) {
      text += Buffer.from(opMatch[1], 'hex').toString('latin1') + ' ';
    } else if (opMatch[2] !== undefined) {
      text += opMatch[2].replace(/\\\(/g, '(').replace(/\\\)/g, ')').replace(/\\\\/g, '\\') + ' ';
    }
  }
  return text;
}
...
test('resolveTaxBreakdown applies Delhi intrastate CGST+SGST and interstate IGST, with POS metadata in the PDF', async function () {
  ...
  const pdfBuffer = await buildInvoicePdfBuffer(transaction);
  const pdfText = extractPdfText(pdfBuffer);
  assert.match(pdfText, /Place of Supply: Delhi \(07\)/);
  assert.match(pdfText, /CGST @ 9%/);
  assert.match(pdfText, /SGST @ 9%/);
  assert.match(pdfText, /Authorized Signatory/);
  assert.match(pdfText, /Customer GSTIN: 07ABCDE1234F1Z5/);
});

test('invoice PDF keeps the brand header, bold TAX INVOICE title, and A4 page size for accounting records', async function () {
  const transaction = {
  ...
  assert.match(pdfText, /TAX INVOICE/);
  assert.ok(pdfText.indexOf('GET READY JOB') < pdfText.indexOf('TAX INVOICE'));
  assert.match(pdfText, /IGST @ 18%/);

  const { PDFDocument } = require('pdf-lib');
  const reloaded = await PDFDocument.load(pdfBuffer);
  assert.equal(reloaded.getPageCount(), 1);
  const { width, height } = reloaded.getPage(0).getSize();
  assert.ok(Math.abs(width - 595.28) < 0.5, 'page width must be A4 (595.28pt)');
  assert.ok(Math.abs(height - 841.89) < 0.5, 'page height must be A4 (841.89pt)');
});
...
test('promo codes: active toggle, public offers list, non-mutating preview, and redeem-only-on-payment', async function () {
  const server = http.createServer(app);
  await new Promise(function (resolve) {
    server.listen(0, '127.0.0.1', resolve);
  });
  ...
  try {
    const createActive = await fetch(baseUrl + '/api/admin/promos', {
      method: 'POST',
      headers: { 'content-type': 'application/json', authorization: 'Bearer ' + token },
      body: JSON.stringify({ code: 'ACTIVE20', discountPercent: 20, usageLimit: 2, active: true, description: '20% off launch offer' })
    });
    assert.equal(createActive.status, 200);

    const createInactive = await fetch(baseUrl + '/api/admin/promos', {
      method: 'POST',
      headers: { 'content-type': 'application/json', authorization: 'Bearer ' + token },
      body: JSON.stringify({ code: 'PAUSED10', discountPercent: 10, active: false })
    });
    assert.equal(createInactive.status, 200);

    // Public offers list must include the active code with safe fields only, and exclude the paused one.
    const offersResponse = await fetch(baseUrl + '/api/public/promo-codes');
    assert.equal(offersResponse.status, 200);
    const offersPayload = await offersResponse.json();
    const codes = offersPayload.offers.map(function (item) { return item.code; });
    assert.ok(codes.includes('ACTIVE20'));
    assert.ok(!codes.includes('PAUSED10'));
    const activeOffer = offersPayload.offers.find(function (item) { return item.code === 'ACTIVE20'; });
    assert.equal(activeOffer.usedCount, undefined);
    assert.equal(activeOffer.description, '20% off launch offer');

    // A paused code must be rejected by validation.
    const pausedValidate = await fetch(baseUrl + '/api/public/promo-codes/validate', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ code: 'PAUSED10', amount: 1000, currency: 'INR' })
    });
    assert.equal(pausedValidate.status, 400);

    // Previewing an active code repeatedly must never mutate its usage count.
    for (let i = 0; i < 3; i++) {
      const previewResponse = await fetch(baseUrl + '/api/public/promo-codes/validate', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ code: 'ACTIVE20', amount: 1000, currency: 'INR' })
      });
      assert.equal(previewResponse.status, 200);
      const previewPayload = await previewResponse.json();
      assert.equal(previewPayload.applied, true);
      assert.equal(previewPayload.finalAmount, 800);
    }

    const afterPreviewList = await fetch(baseUrl + '/api/admin/promos', { headers: { authorization: 'Bearer ' + token } });
    const afterPreviewPayload = await afterPreviewList.json();
    const afterPreviewEntry = afterPreviewPayload.promos.find(function (item) { return item.code === 'ACTIVE20'; });
    assert.equal(afterPreviewEntry.usedCount, 0, 'previewing a code must not consume its usage limit');

    // Completing a real checkout must redeem the code exactly once (create-order alone must not).
    const orderResponse = await fetch(baseUrl + '/api/create-order', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        amount: 1000,
        currency: 'INR',
        receipt: 'active20-order',
        planId: 'lifetime-pro',
        billing: { email: 'active20@example.com', name: 'Active Buyer', country: 'India' },
        promoCode: 'ACTIVE20'
      })
    });
    assert.equal(orderResponse.status, 200);
    const orderPayload = await orderResponse.json();
    assert.equal(orderPayload.success, true);
    assert.equal(orderPayload.localOnly, true, 'test env has no Razorpay key configured, so create-order completes locally');

    const afterOrderList = await fetch(baseUrl + '/api/admin/promos', { headers: { authorization: 'Bearer ' + token } });
    const afterOrderPayload = await afterOrderList.json();
    const afterOrderEntry = afterOrderPayload.promos.find(function (item) { return item.code === 'ACTIVE20'; });
    assert.equal(afterOrderEntry.usedCount, 1, 'a completed local checkout must redeem the code exactly once');

    // Deactivating via admin CRUD must immediately remove it from the public offers list.
    const deactivate = await fetch(baseUrl + '/api/admin/promos', {
      method: 'POST',
      headers: { 'content-type': 'application/json', authorization: 'Bearer ' + token },
      body: JSON.stringify({ code: 'ACTIVE20', active: false })
    });
    assert.equal(deactivate.status, 200);
    const offersAfterDeactivate = await (await fetch(baseUrl + '/api/public/promo-codes')).json();
    assert.ok(!offersAfterDeactivate.offers.map(function (item) { return item.code; }).includes('ACTIVE20'));

    // Flat discounts are entered by the admin in major units (rupees) but must be applied
    // against amounts expressed in minor units (paise) - e.g. flat ₹50 off a ₹249 order.
    await fetch(baseUrl + '/api/admin/promos', {
      method: 'POST',
      headers: { 'content-type': 'application/json', authorization: 'Bearer ' + token },
      body: JSON.stringify({ code: 'FLAT50', discountFlat: 50, active: true })
    });
    const flatValidate = await fetch(baseUrl + '/api/public/promo-codes/validate', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ code: 'FLAT50', amount: 24900, currency: 'INR' })
    });
    assert.equal(flatValidate.status, 200);
    const flatPayload = await flatValidate.json();
    assert.equal(flatPayload.discountAmount, 5000, 'flat 50 rupee discount must equal 5000 paise, not 50 paise');
    assert.equal(flatPayload.finalAmount, 19900);
  } finally {
    await new Promise(function (resolve) {
      server.close(resolve);
    });
