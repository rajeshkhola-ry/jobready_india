...
function loadAppWithTwoFactor(enabled, options) {
  ...
  process.env.ADMIN_2FA_REQUIRED = opts.requireTwoFactor ? 'true' : 'false';
...
test('admin login bypasses 2FA by default and returns admin token', async function () {
  var payload = await login(loadAppWithTwoFactor(true));

  assert.equal(payload.requireOTP, false);
  assert.equal(payload.requires2fa, false);
  assert.ok(payload.token);
  assert.equal(payload.role, 'admin');
});

test('admin login accepts explicitly allowed alternate admin emails without 2FA', async function () {
  var app = loadAppWithTwoFactor(true);

  var payloadForHello = await login(app, 'hello@getreadyjob.com');
  assert.equal(payloadForHello.requireOTP, false);
  assert.ok(payloadForHello.token);

  var payloadForRajesh = await login(app, 'rajesh.khola@gmail.com');
  assert.equal(payloadForRajesh.requireOTP, false);
  assert.ok(payloadForRajesh.token);
});

test('when 2FA is explicitly required, configured setup requests OTP challenge', async function () {
  var payload = await login(loadAppWithTwoFactor(true, { requireTwoFactor: true }));
