...
class _HomePageV11State extends State<HomePageV11> {
    ...
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => CheckoutPage(
          planId: plan,
          billingPeriod: plan,
          amount: _displayAmountForPlan(plan, resolvedCurrency),
          currency: resolvedCurrency,
          activeGateway: _activeGateway,
          usageType: _selectedUsageType,
          sevenDayAmount: _displayAmountForPlan('7Days', resolvedCurrency),
  ...
  /// Activates the Free plan directly (no Promo/GSTIN/Payment checkout involved).
  Future<void> _activateFreePlan() async {
    final profile = UserAccountService.getProfile();
    final freeProfile = profile.copyWith(
      activePlan: 'Free',
      planId: 'free',
      planName: 'Free',
      planStatus: 'active',
      remainingCredits: 3,
      planCurrency: _selectedPaymentCurrency,
      planPrice: 0,
      planSummary: 'Free access to core tools',
      toolsUnlimited: false,
    );
    await UserAccountService.saveProfile(freeProfile);
    await VoiceQuotaService.applyPlanQuotaAllocation('Free');
    ...
    setState(() {});
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Free plan activated. Enjoy core tools right away.')),
    );
  }
          ...
          SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                const _EmbeddedPlanComparisonMatrixSection(),
                const SizedBox(height: 10),
                LayoutBuilder(
...
/// Voice commands strictly consume the user's active plan quota (configured
/// via Admin) - embeds the shared Plan Comparison Matrix directly below the
/// plan cards instead of a separate top-up purchase flow / page visit.
class _EmbeddedPlanComparisonMatrixSection extends StatelessWidget {
  const _EmbeddedPlanComparisonMatrixSection();
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: const PlanComparisonMatrix(embeddedMaxHeight: 560),
    );
  }
}
